...
- **SDR Control タブ**（常時表示・SDR未接続時はパネルをグレーアウト）
  ...
  - **Passband Tune パネル**: ◀◀/◀/▶/▶▶ ボタン + ステップ選択（100Hz〜10kHz）+ オフセット表示 + Reset
    ...
    - **Lock（Lボタン）の帯域幅比スケーリング修正**（2026-07-14・GitHub Issue #14の調査中に発覚）:
      従来の実装（`_doppler_cycle()`のLock分岐、`src/ui/main_window.py`）はダウンリンク側の
      生のHzデルタ（ドップラーシフト量、またはPassband Tuneのオフセット量）を**無変換のまま**
      アップリンク側にミラーリングしており、アップリンク帯域幅とダウンリンク帯域幅が等しいという
      暗黙の前提を置いていた。実際にはRS-44のV/Uリニアトランスポンダーのように**アップリンクと
      ダウンリンクの帯域幅が異なる**衛星があり、その場合は本家GPredictと同様
      「ダウンリンク帯域内での相対位置をアップリンク帯域にスケーリングして写像する」計算
      （= デルタに `アップリンク帯域幅 / ダウンリンク帯域幅` の比を掛ける）が必要。
      `ul_dl_bw_ratio`（`downlink_low/high`・`uplink_low/high`が全て揃っている場合のみ計算、
      いずれか欠けている場合は`1.0`にフォールバックし従来の1:1動作を維持）を導入し、
      Lock分岐・Passband TuneのULミラーリング（Rig1↔Rig2両方向）の計3箇所に適用済み。
      テスト: `tests/test_main_window.py`の`TestLockBandwidthRatio`（帯域幅比1/3の非反転・反転
      両ケース、`_high`欠落時のフォールバック、等幅時の既存動作無変化を検証）
      - **教訓**: このバグは「Radio ControlタブにSDR不要のTuneボタンを追加する」機能
        （最初にIssue #14への回答として実装したが、ユーザー自身が実機（FTX-1F）でLボタンを
        テストして発見・指摘したため、その実装は`git revert`で一旦取り消し、この根本修正を
        先に行った）とは独立した、既存コードの潜在バグだった。新機能を実装する前に、
        関連する既存機能を実機で検証してもらうことの重要性を示す事例
      - **Issue #14自体（無線機VFOダイヤルを手で回したことを検知する機能）は本修正の対象外**。
        検知には`get_frequency()`の定期読み戻しが必要だが、FTX-1Fは`f`/`i`コマンドで
        タイムアウト→通信断・再接続ループに陥る既知の問題があり（本ファイル「FTX-1F 固有の
        制約」参照）、他の対応リグ（FT-991A・IC-9100/9700/705・TS-2000等）についても
        読み戻しの安全性は実機未検証。実装するかどうかは別途判断が必要
...
### f/i（get_freq）が「10秒以上かかる」は誤診断だった（2026-07-14 訂正）

2026-05-20の実機記録は「f/iコマンドはF/I送信直後に10秒以上かかる→使用禁止」としていたが、
これは**FTX-1F/Hamlib側の制約ではなく、`_cmd_raw()`自身の応答解析バグ**が原因と判明した。

rigctldプロトコルは大文字コマンド（`F`・`S`等、書き込み系）の応答は必ず`"RPRT <code>"`だが、
**小文字コマンド（`f`・`i`・`m`等、読み取り系）は成功時に値のみを返しRPRTを含まない**
（エラー時のみ`"RPRT <負の値>"`）。旧`_cmd_raw()`は常に「RPRTが出るまで待つ」実装だったため、
読み取り系コマンドが成功するとその成功を検知できずソケットタイムアウト（10秒）まで
ブロックし続けていた。これは全リグ共通のバグであり、FTX-1F固有ではない
（たまたま読み取り系コマンドが本番コードから一度も呼ばれていなかったため、これまで発現
していなかっただけ）。

**検証方法**: `scripts/test_ftx1_getfreq_net.py`で`S 1 Main`送信の有無・様々な遅延を組み合わせて
実機テストしたところ最初は12/12件が10秒タイムアウトしたが、応答待ちロジックを
「小文字コマンドはRPRTを待たず改行で完結とみなす」に変更したところ、`get_frequency()`が
1〜51msで確実に応答するようになった（実機`HamlibNetController`で4回連続確認済み）。

**修正**: `_cmd_raw()`（`src/rig/controller.py`）に`is_query = command[:1].islower()`判定を追加。
小文字コマンドは`RPRT`を待たず、改行で終わる1行を受信した時点で応答完了とみなす
（大文字コマンドの応答＝RPRT待ちの既存動作は変更なし）。
