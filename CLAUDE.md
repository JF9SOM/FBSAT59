...
- **SDR Control タブ**（常時表示・SDR未接続時はパネルをグレーアウト）
  ...
  - **Rig Tune パネル**（Radio Control タブ、Frequency 枠内・2026-07-14 追加・GitHub Issue #14）
    — Passband Tune と同じUI（◀◀/◀/▶/▶▶ ボタン + ステップ選択100Hz〜10kHz + オフセット表示 + Reset）を、
    **SDR を使わないプレーンなCAT接続リグ**向けに提供する。Tボタン（一回限りの中心周波数リセット）
    だけでは「帯域内をチューニングしつつドップラー追尾を継続する」ことができず、無線機のVFOを
    手動で回すとドップラー補正が次サイクルで問答無用に元へ戻してしまう、という報告
    （GitHub Issue #14 "VFO tracking - user interaction on rig ignored?"）を受けて追加。
    無線機側のVFOを直接手で回したことを検知する仕組みではなく（FTX-1F等一部リグでは
    `get_freq`頻繁呼び出しが使用禁止という制約があり困難）、Passband Tuneと同様に
    **アプリ側のボタン操作でオフセットを持続させる**方式を採る
    - `MainWindow._rig_tune_offset_hz`（`_doppler_cycle()`内、`_sdr_tune_offset`とは完全に独立した別状態）
    - **どちらのリグスロットにもSDRが割り当てられていない場合のみ**有効（`any_sdr`判定）。
      SDRが割り当て済みの場合は既存のSDR Control側Passband Tuneを使う（二重適用を避けるため
      片方のみが常に有効になるよう`any_sdr`で排他制御。両方の値が同時にセットされていても
      無関係な方は無視される設計）
    - Lock（Lボタン）ON時は、オフセットを同一リグ内でULにも反転トランスポンダー考慮の上で
      連動させる（Passband Tune の cross-rig ミラーリングと対をなす、same-rig版のミラーリング）
    - トランスポンダー切り替え時にオフセット自動リセット（Passband Tuneと同じ挙動）
    - テスト: `tests/test_main_window.py`の`TestRigTuneOffset`（rr=0の`_FakeEngine`でDoppler
      シフトを0に固定し、オフセット適用ロジックだけを切り出して検証。SDR併用時の既存Passband Tune
      挙動が回帰していないことも合わせて確認）
