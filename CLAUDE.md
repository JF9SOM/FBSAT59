...
- **ローテータ更新周期のプルダウン選択**（2026-09-15、選択肢は同日中に0.1秒→2秒へ変更）— 無線機コントロールの「ローテータに接続」ボタンと「South Init」チェックボックスの間に「Cycle」プルダウン（当初は5秒/2秒/1秒/0.5秒、2026-09-19以降は**5秒/4秒/3秒/2秒/1秒**、既定1秒）を追加。従来ローテータへのAZ/EL送信は表示更新用の固定1秒`self._timer`に相乗りしていたが、`self._rotator_timer`という専用タイマーに分離し（世界地図等の表示更新負荷とは無関係に）間隔を可変化した。送信ロジック自体（`_send_to_rotator()`、非ブロッキングのバックグラウンドスレッド送信）は変更なし。選択値は`app_settings`の`rotator_cycle_ms`キーに永続化し次回起動時も復元（`MainWindow._load_rotator_cycle_setting()`/`_on_rotator_cycle_changed()`、`RadioControlWidget.set_rotator_cycle()`）。
  **0.5秒選択肢も2026-09-19に削除**（実運用で現実的な周期ではないとのユーザー判断。DBに`500`が保存されていた場合は既定の1000msにフォールバック）。
  **0.1秒選択肢を削除した経緯**: 実機ログ分析の過程で、SkyWatcherローテーター（Hamlib rot model 2801）は`set_position()`1回につき約10往復のシリアルトランザクションを行うプロトコルで、Windows実機ベンチマーク（`docs/hamlib.md`「HamlibRotatorController — Catch-up タイムアウト設計」節）実測値ベースだと1サイクル（`set_position`+`get_position`）が約80〜170msかかることが判明。0.1秒（100ms）はこの実測往復時間と同程度かそれ以下で、`_rot_busy_lock`により多くのtickが黙ってスキップされ、公称値どおりの間隔は事実上実現できないと判断しユーザー了承のもと削除した。
- **Rig CAT / Doppler「Cycle」もプルダウン化**（2026-09-15）— 従来は数値を手入力/スピン矢印で指定する`QSpinBox`（10〜10000ms任意値）だったが、ローテータCycleと同じ方式のプルダウン（`_cycle_combo`、当初は2秒/1秒/0.5秒/0.1秒、2026-09-19以降は**3秒/2秒/1秒/0.5秒**、既定1秒。CATコントロールでは0.1秒周期は実現不可というユーザー判断で0.1秒を削除し3秒を追加。DBに`100`が保存されていた場合は既定の1000msにフォールバック）に統一。ウィジェット名も`_cycle_spin`→`_cycle_combo`に改名。`DopplerWorker`（`_doppler_cycle`）を駆動する間隔という役割・DBキー（`rig_cycle_ms`）・`MainWindow._load_cycle_setting()`/`_on_cycle_changed()`は変更なし。許可値は`_CYCLE_CHOICES_MS`（main_window.py）で管理し、DB内の値がこの4値以外だった場合は既定の1000msにフォールバックする。
- **ローテータcatch-up初回jumpの先読み（lead-time）ターゲティング**（2026-09-16）— 実機ログ分析で、初回jumpが接続時点の静的な衛星位置を1回送るだけで、catch-up終了まで（実測約48秒）Pコマンドを再送信しないため、衛星が動き続けた分だけ「たまたますれ違って」catch-up成立と誤判定される問題が判明。`HamlibRotatorController`に`RotatorController.set_predictor()`で軌道予測コールバックを注入できるようにし、初回jump時は「衛星の現在位置」ではなく「意図的に控えめな想定スルー速度（`_CATCH_UP_LEAD_ASSUMED_DEG_PER_S=0.7`度/秒、上限`_CATCH_UP_LEAD_MAX_S=120`秒）で見積もった先読み時間だけ未来の、衛星の実経路上の地点」へPコマンドを送るよう変更。目標地点が水平線下（EL<0）なら動かさず次サイクルへ持ち越す。Pコマンド自体は引き続き1回きり（過去に「毎tick再送信」を試して`96a630d`→`c2e773a`で撤回した経緯があり、SkyWatcherの`:K1 :K2...`軸停止を含む約10往復トランザクションのため頻繁な再送はモーターが加速しきれず逆効果になるとみられる）。詳細設計・なぜ速度実測が不要かの理屈は[docs/hamlib.md](docs/hamlib.md)「初回jumpの先読み（lead-time）ターゲティング」節参照。catch-up終了判定も同日中に「衛星の生位置」基準から「送った目標（`_last_az`）」基準へ修正（前者だと衛星が動き続けた分だけ目標に届く前に誤って終了してしまうため）。さらに0度またぎでのcatch-up再突入（`crossed_zero`）にも同じ先読みを適用（未適用のままだと、まさに0度またぎが起きやすい天頂通過付近のTCAで目標が一瞬で無意味になり、実機テストで60秒間完全停止する症状が確認されたため）。詳細は[docs/hamlib.md](docs/hamlib.md)「0度またぎのcatch-up再突入にも先読みを適用」節参照。

**2026-09-17: 0度またぎcatch-upの「出発地点からの離脱距離」ゲート追加** — 上記の先読み修正後も、実機テスト（最大仰角70度・北跨ぎパス）で北跨ぎのcatch-upが目標へ一気に回らず、通常追尾のような「2秒動いては止まる」動きになる症状が判明。原因はcatch-up終了判定の`az_diff`計算（最短角度距離）が**ローテーターの実際の回転方向を考慮しない**ため——北跨ぎは出発点と目標が0/360の縫い目を挟んで数値的に隣接する構造上、ローテーターが出発直後（まだほとんど動いていない）でも最短角度距離だけを見ると「目標に近い」と誤判定してしまい、catch-upが即座に終了していた（実機ログでは送信2秒後の1回目のポーリングで誤終了）。修正として、北跨ぎ再突入の瞬間のローテーター実位置を`_catch_up_wrap_origin_az`として記録し、そこから`_CATCH_UP_WRAP_DEPARTURE_MARGIN_DEG`（10度）以上離れるまで到達判定自体をスキップするゲートを追加（`HamlibRotatorController`）。時間ベースではなく距離ベースにしたことで、想定スルー速度という新たな定数を導入せずに済んでいる。0度またぎ再突入（`crossed_zero`）のみに適用され、初回jumpには影響しない。詳細は[docs/hamlib.md](docs/hamlib.md)「0度またぎcatch-upの『出発地点からの離脱距離』ゲート」節参照。

**2026-09-17: catch-upタイムアウト機構を完全撤廃・想定スルー速度を1.0度/秒へ引き上げ** — 通常の初回接続（0度またぎ非経由）の実機テストで、先読み目標が遠すぎて`_CATCH_UP_TIMEOUT`（60秒）が2回連続で誤発動し、catch-up成立まで約132秒かかる症状が判明。根本原因は`_CATCH_UP_TIMEOUT`（60秒）が`_CATCH_UP_LEAD_MAX_S`（120秒）より短く、正当に先読み目標へ向かっている最中のローテーターを毎回打ち切ってしまう設計上の矛盾。さらに検討の結果、catch-up終了判定を「送った固定目標」基準へ既に変更済み（前述）のため、タイムアウトが本来守っていた「衛星と同方向で永遠に追いつけない」問題自体がすでに解消しており、**タイムアウト（および代替案として検討したスタール検知）は、正当に進行中のcatch-upを誤って妨害するリスクしかない**と判断し、`_CATCH_UP_TIMEOUT`・タイムアウト再送信処理を完全に削除（`HamlibRotatorController`）。catch-upは目標との差が5度以内になるまで無期限に待ち続ける設計に変更。本当にローテーターが機械的に動かなくなった場合の復旧は、レーダー画面のX印が動かないことにユーザーが気づき、手動移動または再接続でリセットする運用に委ねる。あわせて`_CATCH_UP_LEAD_ASSUMED_DEG_PER_S`を`0.7`→`1.0`度/秒に引き上げ（実測値約1.1〜1.2度/秒より依然安全な範囲を保ちつつ先読み距離を約3割短縮）。詳細は[docs/hamlib.md](docs/hamlib.md)「タイムアウト機構の完全撤廃・想定スルー速度の引き上げ」節参照。

**2026-09-17（同日）: 先読み距離の計算をHamlibが強制する「長い方の経路」基準に修正・先読み時間上限を撤廃** — 上記対応後もなお、北跨ぎ直後にcatch-upが完全に詰まる（無期限に待ち続けるが二度と進展しない）実機ケースが発覚。原因はユーザーの指摘により判明: SkyWatcher実機自体は0/360度をまたいで連続回転できるが、**Hamlibのこの機種向けバックエンドはコマンドとして送れる方位角を0〜360度の範囲に制限しており**、出発地点と目標が0/360の縫い目をまたぐ場合、最短経路（数値上小さい方）は0度未満または360度超の値を一時的に必要とするため送れず、**逆方向・長い方の経路（ほぼ1周分）への回転を強制される**（実機の物理制約ではなくHamlibバックエンドのソフトウェア制限で、アプリ側からは変更不可）。`_lead_target()`は最短経路の距離で先読み時間を計算していたため、境界をまたぐケース（出発0.1度、目標360.0度のような）では最短経路がわずかな値になり、「実質動く必要のない目標」を計算してしまい、離脱距離ゲートが要求する10度以上の離脱が永遠に起きずcatch-upが詰まっていた。修正として、出発地点と目標が境界をまたぐ場合（`crossed_zero`と同じ判定条件）は先読み距離を`360度 − 最短経路の距離`（長い方の経路）に置き換え、初回jump・0度またぎ再突入の両方に自動適用されるようにした。あわせて`_CATCH_UP_LEAD_MAX_S`（先読み時間上限120秒）を完全に撤廃（想定スルー速度1.0度/秒に対し120秒では120度分しか先読みできず、境界またぎで必要な最大約360度弱に全く足りなかったため。水平線下保護は別の独立したチェックが既に担っており上限自体が不要だった）。詳細は[docs/hamlib.md](docs/hamlib.md)「先読み距離の計算に『Hamlibが強制する長い方の経路』を使うよう修正・先読み時間上限を撤廃」節参照。

**2026-09-17（同日）: 想定スルー速度を自動計測・永続化する仕組みに変更** — 上記の北跨ぎ実機ログ（21:01接続）を分析したところ、初回jumpからcatch-up成立まで実測で約142〜148度を約51.8秒（約2.7〜2.9度/秒）で移動していたことが判明し、当時の想定スルー速度1.0度/秒はcatch-upの全速力移動を見積もるにはかなり遅すぎることが裏付けられた。実スルー速度はローテーター機種・搭載アンテナの重さ/風荷重でインストールごとに異なるため、`_CATCH_UP_LEAD_ASSUMED_DEG_PER_S`という固定クラス定数を廃止し、`RotatorController._assumed_slew_deg_per_s`という**インスタンス変数**（基底クラス既定値2.5度/秒）に変更。catch-up episode（初回jump・0度またぎ再突入とも）の開始時刻・開始位置を記録しておき、catch-up終了時に`HamlibRotatorController._record_measured_slew_speed()`が「移動角度÷経過時間」で実速度を自動算出、**実測値×`_SLEW_SPEED_SAFETY_MARGIN`（0.8）**を新しい想定値として採用する（ユーザー了承の初期値。遅すぎるようなら後日0.9等に引き上げる想定）。想定スルー速度は「実速度を絶対に過大評価してはいけない」という設計前提があるため、1回の測定をそのまま信用せず安全マージンを掛けている。測定値は`MainWindow`が`app_settings`の`rotator_slew_deg_per_s`キーへ永続化し次回起動時も復元（`RotatorController.set_assumed_slew_speed()`/`set_slew_speed_observer()`という新しい基底クラスフックのペア、`set_predictor()`と対称的な設計。観測コールバックはローテーターI/Oのバックグラウンドスレッドから呼ばれるため、Qtシグナル`_rot_slew_speed_measured`でUIスレッドへ引き渡してからDB書き込み）。catch-up完了時の実測ログ（出発地点・到達地点・移動距離・経過時間・実測値・マージン適用後の値）は`fbsat59.log`本体を肥大化させないよう専用の`rot-record.log`（`src/rig/rot_record_log.py`、`src/sdr/diag_log.py`と同じ`propagate=False`パターン）に分離して記録する。詳細は[docs/hamlib.md](docs/hamlib.md)「想定スルー速度の自動計測・永続化」節参照。

**2026-09-17（同日）: 上記の自動計測ロジックが「長い方の経路」を考慮しておらず実速度を大幅に過小評価するバグを発見・修正** — 上記機能の実装直後に北跨ぎパスで実機テストしたところ動作自体（0度を跨いだ瞬間に全速力で反対回りし目標へ到達）は成功したが、`rot-record.log`とDBを確認すると`rotator_slew_deg_per_s=0.218`という明らかに低すぎる値が保存されていた。原因は`_record_measured_slew_speed()`が移動距離を常に最短経路（22.3度）で計算していたためで、実際には`_lead_target()`が`boundary_forced`と判定し長い方の経路（約337.6度）を先読み距離に使ったケースだったため、実速度（約4.1度/秒）を約1/15に過小評価していた。放置すれば次回catch-upの先読み時間が10倍以上長くなり機能追加の目的が台無しになるところだった。修正として、catch-up開始時に`_lead_target()`と全く同じ`boundary_forced`条件を`set_position()`側でも算出し`_catch_up_measure_long_path`として記録、`_record_measured_slew_speed()`完了時にこのフラグを見て該当する場合は`360度−最短経路`を実移動距離として使うようにした。あわせて、`get_rotor_record_logger()`が`pytest`実行時も本番と同じ実ログファイルに書き込んでしまい、テストの架空の数値が実機記録に混入していた問題も発見し、`TestHamlibRotatorController`にautouseフィクスチャを追加してテストが実ログファイルに触れないよう修正。誤って永続化された値はDBから削除し既定値2.5に戻した。詳細は[docs/hamlib.md](docs/hamlib.md)「上記の計測ロジックに、長い方の経路を考慮していないバグ」節参照。

**2026-09-17（同日）: `_lead_target()`が「水平線下で保留」と判定した際の診断ログを追加** — 実機テスト中、ローテーター電源を入れ忘れた状態での接続試行（`_probe_rotator_open()`のクラッシュ隔離が正常動作しSIGSEGVを吸収）の直後、再接続自体はソフト的に成功するのに以降ずっと`lead target below horizon, holding`が出続けて動かなくなる症状が発生。TLEで実際の衛星仰角を検証したところ通常のジオメトリでは説明がつかない一方、当時のログには`_lead_target()`が実際に使った出発地点（ローテーター実位置）・衛星生位置・先読み時間・予測AZ/ELのいずれも残っておらず数値で原因を特定できなかった（電源再投入でSkyWatcher内部の相対位置カウンターがリセットされた可能性を疑ったが未確証）。動作変更なしで、`_lead_target()`が水平線下と判定した際にこれらの値を`INFO`ログへ出力するようにし、あわせて初回jump・0度またぎ再突入の成功ログにも出発地点を追加した。詳細は[docs/hamlib.md](docs/hamlib.md)「`_lead_target()`が『水平線下で保留』と判定した際の診断ログ追加」節参照。

**2026-09-18: 水平線下の衛星への「次のAOSへ先回り」機能を追加** — 上記の北跨ぎ調査の過程で、LOS後（あるいは接続時にまだAOS前）は`_lead_target()`が水平線下と判定するたびに完全に静止し続ける（通常追尾中のようにEL=0クランプでAZだけ追従することすらしない）という別の設計上の制約が判明。ユーザー承認の新仕様: (1) 水平線下の衛星でConnectしたら、そのAOS地点のAZ（EL=0）へ先読みなしで一度だけPコマンドを送り全速力移動・対象ELが0超になったら通常追尾開始、(2) LOSを迎えたら次パスのAOS地点へ全速力移動し以降は(1)と同じ、(3) `crossed_zero`再突入時に先読み目標が水平線下ならそのパスの追尾を打ち切り(2)と同じ動作、(4) 初回jumpの先読み目標が水平線下の場合も(3)と同様、(5) Autotrack有効時は現在選択中の衛星ではなくAutotrackリスト内で最も早くAOSを迎える衛星を基準にする。実装は`RotatorController.set_next_aos_predictor()`（`set_predictor()`と同じ注入パターン）＋`MainWindow._predict_next_aos_az()`（`PassPredictor.get_passes(min_elevation_deg=0.0)`の先頭パスのAOS方位角）＋`HamlibRotatorController`の`_awaiting_aos`フラグと`_start_aos_wait()`。`AutotrackManager.check()`のRule 2b（[src/core/autotrack.py](src/core/autotrack.py)）が対象衛星の昇る前から`_selected_norad`を先回りして切り替えていることを確認できたため、ローテーター制御層はAutotrackを一切意識せず要件(5)も満たせる設計にした。既存のcatch-up到達判定・`rot-record.log`ポーリング・スルー速度計測をそのまま流用（AOS地点は固定点なので先読み時間計算は不要）。実機での動作確認は未実施。詳細は[docs/hamlib.md](docs/hamlib.md)「水平線下の衛星への『次のAOSへ先回り』機能の追加」節参照。

**2026-09-19: ローテーター未接続なのに「Connected」（緑）になるバグを修正・「未接続」表示と「Use Rotator」チェックを追加** — アダプタごと未接続でAutotrackのIQ録音を走らせたところ、ローテーター欄が緑の「Connected」になり約7分間存在しない機器へ位置コマンドを送り続けた。原因はHamlibのPythonバインディングが`Rot.open()`/`set_position()`/`get_position()`の失敗で**例外を投げず`rot.error_status`にしか書かない**のに、`HamlibRotatorController`だけがそれを見ていなかったこと（リグ側は`_check_rig_ok()`で対処済み）。接続前確認のプローブサブプロセスも存在しないポートに`ok:true`を返していた。修正: 新状態`RigState.UNREACHABLE`（Radio Controlの「Rotator:」欄が赤の**「未接続」**）、`open()`後の`error_status`確認、途中切断の検出（`set_position`/`get_position`の失敗が成功を挟まず**30秒間**続いたときだけ`UNREACHABLE`へ。短い失敗は無視。NETは空応答/`RPRT`負値）、Autotrack/Recordダイアログに**「Use Rotator」チェック**（既定ON。OFFならAOSで自動接続せずLOSでも切断しない。未接続時は灰色「使用しない」）。UNREACHABLE中は`is_connected`がFalseなので追尾送信・ポーリングは既存ガードで止まる。Rig 1/2（SDR含む）の接続失敗表示も同じ赤「未接続」に統一（ボタンも「Retry」→「Connect Rig N」、表示のみの変更）。手動Connectでの「未接続」表示は実機確認済み。**正常応答する実ローテーターでの追尾中の誤検出有無は未検証**。詳細は[docs/hamlib.md](docs/hamlib.md)「ローテーター未接続なのに『Connected』（緑）になるバグ」節参照。
