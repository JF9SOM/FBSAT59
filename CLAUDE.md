...
### 15. Autotrackトグル時の「再起動を促すポップアップ」— 根本原因未解決のまま運用で吸収する対症療法（GitHub Issue #27、2026-08-23 追加）

**経緯**: 項目14（min_el〜真の地平線間のグレーゾーンでの誤切替）を修正しリリースした後も、
2人目の報告者（on7ndr、Windows）から「METEORウィンドウでRTL-SDRの検出が不安定
（"no device"・"WinError 6"）、AOSでは自動起動しないがLOSでは正しく停止・処理される」
という新しい症状が報告された。LOS側が正常に動くことから、Autotrackの追跡判定自体
（衛星選択・LOS検出）は機能しており、**AOS時のSatDump起動（RTL-SDRデバイスのオープン）
だけが失敗する**という、項目14とは別の症状だと判明した。有力な仮説（未検証）として、
`MeteorTab._disconnect_sdr()`のバグ（本ファイル前述、実際には何も切断しない）と、
Rig 1/2にRTL-SDRを割り当てている場合の`_autotrack_on_aos()`（Rig自動接続の直後に
METEORタブを起動）との競合を提示したが、開発者（JF9SOM）から「これ以上の原因調査・
