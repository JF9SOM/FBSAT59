...
ヒューリスティックとして新設（`projected`=3 > `_map`=2 > `corrected`=1 >
それ以外=0。地図投影済みの合成画像を最優先、次に地図オーバーレイ付き、
次に色補正済み、それ以外は最下位）し、**ライブ受信（`_on_new_image()`）・
過去フォルダ読み込み（`_load_images_from_folder()`）の両方で共用**する。
