...
### Lock（Lボタン）— Dial Feedback設計（2026-07-15 実装・GitHub Issue #14・NETモードのみ）

#### 背景・GPredict本家との比較

GitHub Issue #14「VFOダイヤルを手で回してもドップラー補正が問答無用に上書きする」への対応。
本家GPredict（csete/gpredict、`src/gtk-rig-ctrl.c`）の実ソースを確認したところ:

- **`track_downlink()`**: `delta = down - downlow; up = uplow + delta`（反転時は`up = uphigh - delta`）
  という**生のHzデルタをそのまま適用**する実装で、帯域幅比によるスケーリングは一切行っていない。
  一時的に「UL/DL帯域幅が異なる場合は比率でスケーリングすべき」という仮説を検証・実装したが
  （commit `9aa2201`）、GPredictの実際の挙動と矛盾すると判明し**revertした**（commit `f3b0a95`）。
  FBSAT59の元々の実装（生デルタ）が最初からGPredictと一致していた
- **Dial feedback**: `get_freq_simplex()`でrigctldに`"f\n"`（またはVFOオプション時`"f currVFO\n"`）を
  毎サイクル送信し、直前に自分が送信した周波数（`ctrl->lastrxf`）と1Hz以上差があれば
  「ユーザーがダイヤルを回した」と判定、`track_downlink()`でアップリンクを再計算する設計

この`get_freq_simplex()`が送るコマンドは、上記の`_cmd_raw()`バグで長年「FTX-1Fでは使用禁止」と
誤診断されていた`f`コマンドそのものであり、バグ修正によってようやく実装可能になった。

#### 実装方針（ユーザー判断・2026-07-15）

以下2点をユーザーの明示的な判断で確定:
1. **帯域幅比スケーリングは行わない**（GPredictに合わせ生デルタ方式のまま）
...
区別しないため、この方法で読み戻し検知ロジックを妥当性検証できる）

#### 未対応・今後の課題

- **Directモード（`HamlibDirectController`）は未対応**（`isinstance`チェックで明示的に除外）。
  Hamlibの`get_freq()`がDirectモードの各リグ（FTX-1F・FT-991A・IC-9100/9700/705等）で
  安全に高速動作するかは別途検証が必要
