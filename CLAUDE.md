...
SWIGコマンドに1フラグを足して同じ手順で再ビルドするだけで済む。このプロジェクトは
既にCAT操作全体を`_rig_cmd_lock`で直列化しているため、「複数スレッドが同時にHamlibを
叩く」ことへの懸念はなく、`-threads`が変えるのは「GIL解放中に**他の**Pythonスレッド
（音声コールバック等）が動けるかどうか」だけ。ビルドし直すこと自体はCIで完結するが、
**実機（IC-9700）での検証はまだ行っていない**——SWIGの`-threads`は一般に安全とされる
パターン（純Cライブラリの同期API呼び出し、コールバックでPythonへ戻らない）だが、
実際に動作が変わらないか（特にDL/ULスロットリングとの組み合わせ）は次回リリースでの
実機確認が必要。
