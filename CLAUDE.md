...
1e. **Tuneボタンがバンド中心ではなく`downlink_low`（下限）に固定されている疑い（2026-08-12発見、対応予定: 翌日）** — `_refresh_radio_control()`（[main_window.py](src/ui/main_window.py)、衛星選択時に実際に使われるトランスポンダー取得経路）の生SQLが列を明示指定しており、`downlink_high`・`uplink_high`を選択していない。`_on_tune_requested()`は`dl_high = self._current_transmitter.get("downlink_high")`を読んでバンド中心（`(dl_low+dl_high)/2`）を計算する設計だが、この経路で構築された`_current_transmitter`辞書には`downlink_high`/`uplink_high`キー自体が存在しないため常に`None`が返り、`if dl_low is not None and dl_high is not None:`（バンド中心）の分岐に一度も入れず、`elif dl_low is not None:`（バンド下限のみ）に常にフォールバックしている可能性が高い。Issue #18の`rx_offset_hz`追加時に同じSELECT文を修正した際、列の見落とし自体は別件として発見（本ファイル「バグ修正 — 衛星を切り替えて戻るとOffsetが消える」参照）。`TransmitterManager.get_transmitters()`（`SELECT *`）経由の他画面（Comms Quick Panel等）は影響を受けない。対応方針: SELECT列リストに`downlink_high`・`uplink_high`を追加し、実際にTuneボタンを押した際の挙動（帯域幅を持つトランスポンダーでバンド中心に来るか）を確認すること
