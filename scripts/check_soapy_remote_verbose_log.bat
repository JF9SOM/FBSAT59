@echo off
REM FBSAT59 Remote SDR verbose-log diagnostic launcher (Windows, GitHub Issue #12)
REM
REM Runs check_soapy_remote_verbose_log.py in the same folder, using
...
echo.
echo FBSAT59 Remote SDR verbose-log diagnostic
echo ============================================
echo.
...
where py >nul 2>nul
if %errorlevel%==0 (
    py -3.11 "%~dp0check_soapy_remote_verbose_log.py" %SDR_HOST% %SDR_PORT%
    goto :done
)

where python >nul 2>nul
if %errorlevel%==0 (
    python "%~dp0check_soapy_remote_verbose_log.py" %SDR_HOST% %SDR_PORT%
    goto :done
)
