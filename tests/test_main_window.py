...
class _FakeEngine:
    """Fake SatelliteEngine that always returns range_rate_km_s=0 (no Doppler shift),
    so dl_corr/ul_corr equal the transmitter's nominal frequencies exactly —
    isolating the Rig Tune offset math from Doppler shift math in assertions."""
...
class TestRigTuneOffset:
    """Rig Tune offset tests (GitHub issue #14) — _doppler_cycle()'s handling of
    _rig_tune_offset_hz for plain CAT rigs when no SDR is assigned."""
    ...
    def _make_window(self, qtbot, db):
        ...
        w._selected_norad = 25544
        w._current_transmitter = dict(self._XPDR)
        return w

    def test_rig_tune_offset_applied_to_rig1_dl_when_no_sdr(self, qtbot, db) -> None:
        w = self._make_window(qtbot, db)
        rig = _FakeRig(is_sdr=False, is_connected=True)
        w._rig_controller = rig
        w._rig_tune_offset_hz = 1_000.0
        w._doppler_cycle()
        qtbot.waitUntil(lambda: len(rig.calls) > 0, timeout=2000)
        dl, ul = rig.calls[0]
        assert dl == 145_800_000.0 + 1_000.0  # downlink_low + offset (rr=0 so no Doppler shift)
        assert ul == 435_000_000.0  # Lock is off — UL untouched

    def test_rig_tune_offset_mirrors_to_ul_when_locked(self, qtbot, db) -> None:
        w = self._make_window(qtbot, db)
        rig = _FakeRig(is_sdr=False, is_connected=True)
        w._rig_controller = rig
        w._rig_tune_offset_hz = 1_000.0
        ...
        qtbot.waitUntil(lambda: len(rig.calls) > 0, timeout=2000)
        dl, ul = rig.calls[0]
        assert dl == 145_800_000.0 + 1_000.0
        assert ul == 435_000_000.0 + 1_000.0

    def test_rig_tune_offset_inverted_for_inverting_transponder(self, qtbot, db) -> None:
        w = self._make_window(qtbot, db)
        w._current_transmitter = dict(self._XPDR, invert=True)
        rig = _FakeRig(is_sdr=False, is_connected=True)
        w._rig_controller = rig
        w._rig_tune_offset_hz = 1_000.0
        w._trsp_lock = True
        ...
        dl, ul = rig.calls[0]
        assert dl == 145_800_000.0 + 1_000.0
        # Lock+invert computes UL from uplink_high (not uplink_low) — see the
        # Lock branch in _doppler_cycle(); rig_tune still mirrors with the sign
        # reversed for an inverting transponder.
        assert ul == 435_150_000.0 - 1_000.0

    def test_rig_tune_offset_not_applied_when_sdr_assigned(self, qtbot, db) -> None:
        """When Rig 1 is an SDR, _sdr_tune_offset (Passband Tune) governs, not
        _rig_tune_offset_hz — the two controls must never both apply to the
        same rig in the same cycle."""
        w = self._make_window(qtbot, db)
        rig = _FakeRig(is_sdr=True, is_connected=True)
        w._rig_controller = rig
        w._rig_tune_offset_hz = 1_000.0  # set but must be ignored: an SDR is assigned
        w._sdr_tune_offset = 0.0
        w._doppler_cycle()
        qtbot.waitUntil(lambda: len(rig.calls) > 0, timeout=2000)
        dl, ul = rig.calls[0]
        assert dl == 145_800_000.0  # unaffected by _rig_tune_offset_hz
        assert ul == 435_000_000.0

    def test_sdr_tune_offset_still_works_unchanged(self, qtbot, db) -> None:
        """Regression check: existing SDR Passband Tune behaviour (Rig 1 = SDR,
        Rig 2 mirrors via Lock) is unaffected by the Rig Tune addition."""
        w = self._make_window(qtbot, db)
        rig1 = _FakeRig(is_sdr=True, is_connected=True)
        rig2 = _FakeRig(is_sdr=False, is_connected=True)
        w._rig_controller = rig1
        w._rig2_controller = rig2
        w._sdr_tune_offset = 2_000.0
        w._trsp_lock = True
        w._doppler_cycle()
        qtbot.waitUntil(lambda: len(rig1.calls) > 0 and len(rig2.calls) > 0, timeout=2000)
        dl1, ul1 = rig1.calls[0]
        assert dl1 == 145_800_000.0 + 2_000.0
        assert ul1 == 435_000_000.0  # Rig 1 is the SDR: its own UL is untouched
        dl2, ul2 = rig2.calls[0]
        assert dl2 == 145_800_000.0  # Rig 2 is not the SDR: DL untouched
        assert ul2 == 435_000_000.0 + 2_000.0  # mirrored from Rig 1's SDR tune
