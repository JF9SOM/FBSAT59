...
from core.engine import DopplerCalculator, Observation, PassInfo
...
class _FakeRig:
    """Minimal fake RigController for _doppler_cycle() tests (GitHub issue #14)."""

    def __init__(self, is_sdr: bool = False, is_connected: bool = True) -> None:
        self.is_sdr = is_sdr
...
class _FakeEngine:
    """Fake SatelliteEngine returning a fixed, caller-chosen range_rate_km_s,
    so the resulting Doppler-corrected downlink offset from downlink_low is
    known and non-zero — used to drive the Lock bandwidth-ratio scaling
    through its real code path (not a button override, which is consumed
    *after* the Lock branch already ran — see _doppler_cycle())."""

    def __init__(self, range_rate_km_s: float) -> None:
        self._rr = range_rate_km_s
    ...
    def observe(self, norad: int) -> Observation:
        return Observation(
            ...
            azimuth_deg=180.0,
            range_km=1000.0,
            range_rate_km_s=self._rr,
            is_above_horizon=True,
        )


class TestLockBandwidthRatio:
    """Lock (L button) uplink/downlink bandwidth-ratio scaling (GitHub issue
    #14): the uplink offset must scale by (uplink_bw / downlink_bw), not
    mirror the downlink's raw Hz delta 1:1, whenever the transponder's
    uplink and downlink passbands differ in width."""

    _RR = -5.0  # km/s, arbitrary nonzero approaching velocity

    def _make_window(self, qtbot, db, xpdr: dict):
        from data.tle_manager import TLEManager
        from ui.main_window import MainWindow
        ...
        w = MainWindow(conn=db, tle_manager=tle_manager)
        qtbot.addWidget(w)
        w._engine = _FakeEngine(self._RR)
        w._selected_norad = 25544
        w._current_transmitter = dict(xpdr)
        w._trsp_lock = True
        return w

    def _dl_delta(self, dl_low: float) -> float:
        """The exact Doppler shift _doppler_cycle() will compute for dl_low
        at self._RR — ground truth for assertions, not a hand-picked value."""
        return DopplerCalculator.shift_hz(dl_low, self._RR)

    def test_scales_by_bandwidth_ratio_noninverting(self, qtbot, db) -> None:
        """Downlink 30 kHz wide, uplink 10 kHz wide (ratio 1/3): the uplink
        offset must be 1/3 of the downlink's Doppler offset from
        downlink_low, not mirrored 1:1."""
        xpdr = {
            "downlink_low": 435_600_000,
            "downlink_high": 435_630_000,  # 30 kHz wide
            "uplink_low": 145_900_000,
            "uplink_high": 145_910_000,  # 10 kHz wide
            "invert": False,
            "mode": "USB",
        }
        w = self._make_window(qtbot, db, xpdr)
        rig = _FakeRig(is_sdr=False, is_connected=True)
        w._rig_controller = rig
        w._doppler_cycle()
        qtbot.waitUntil(lambda: len(rig.calls) > 0, timeout=2000)
        dl, ul = rig.calls[0]
        delta = self._dl_delta(435_600_000)
        assert dl == pytest.approx(435_600_000 + delta)
        assert ul == pytest.approx(145_900_000 + delta / 3.0)

    def test_scales_by_bandwidth_ratio_inverting(self, qtbot, db) -> None:
        """Same 3:1 downlink:uplink bandwidth ratio, but inverting: the
        scaled offset must apply from uplink_high downward instead."""
        xpdr = {
            "downlink_low": 435_600_000,
            "downlink_high": 435_630_000,  # 30 kHz wide
            "uplink_low": 145_900_000,
        ...
        delta = self._dl_delta(435_600_000)
        assert dl == pytest.approx(435_600_000 + delta)
        assert ul == pytest.approx(145_910_000 - delta / 3.0)

    def test_falls_back_to_1to1_when_band_edges_missing(self, qtbot, db) -> None:
        """Community-transmitter-style entries with only downlink_low/
        uplink_low (no _high) can't compute a bandwidth ratio — Lock must
        fall back to the previous 1:1 raw-delta behaviour rather than crash
        or silently drop the uplink."""
        xpdr = {
        ...
        qtbot.waitUntil(lambda: len(rig.calls) > 0, timeout=2000)
        dl, ul = rig.calls[0]
        delta = self._dl_delta(435_612_000)
        assert dl == pytest.approx(435_612_000 + delta)
        # No uplink_high to anchor an inverted calc against -> falls back to
        # the ul_low + delta branch even though invert=True (matches the
        # pre-existing behaviour: the `invert and ul_high is not None` guard
        # requires ul_high), with the ratio itself also falling back to 1.0.
        assert ul == pytest.approx(145_993_000 + delta)

    def test_equal_bandwidths_unaffected(self, qtbot, db) -> None:
        """Regression check: when uplink and downlink bandwidths are equal
        (the common case for most transponders), the ratio is 1.0 and
        behaviour is identical to the old unscaled code."""
        xpdr = {
            "downlink_low": 145_800_000,
            "downlink_high": 145_950_000,  # 150 kHz wide
            "uplink_low": 435_000_000,
            "uplink_high": 435_150_000,  # 150 kHz wide
            "invert": False,
            "mode": "FM",
        }
        w = self._make_window(qtbot, db, xpdr)
        ...
        qtbot.waitUntil(lambda: len(rig.calls) > 0, timeout=2000)
        dl, ul = rig.calls[0]
        delta = self._dl_delta(145_800_000)
        assert dl == pytest.approx(145_800_000 + delta)
        assert ul == pytest.approx(435_000_000 + delta)
