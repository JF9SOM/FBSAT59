...
class TestDialFeedback:
    """Dial feedback tests (GitHub issue #14) — _apply_dial_feedback() and the
    reset points for _dial_feedback_offset_hz. Verified against GPredict's
    actual track_downlink()/dial-feedback source (2026-07-15): a manual VFO
    retune, detected via reading the downlink back and comparing to what was
    just commanded, accumulates as a persistent offset while Lock is on."""
    ...
    def _make_net_rig(self):
        """A HamlibNetController with a mocked socket — _apply_dial_feedback()
        gates on isinstance(rig, HamlibNetController), so a plain fake object
        won't do."""
        from unittest.mock import MagicMock

        from rig.controller import HamlibNetController

        rig = HamlibNetController(host="localhost", port=4532)
        mock_sock = MagicMock()
        rig._sock = mock_sock
        return rig, mock_sock

    def test_ignores_non_net_rig(self, qtbot, db) -> None:
        w = self._make_window(qtbot, db)

        class _NotNetRig:
            is_sdr = False

        w._apply_dial_feedback(_NotNetRig(), 145_800_000.0)  # type: ignore[arg-type]
        assert w._dial_feedback_offset_hz == 0.0

    def test_ignores_sdr_rig(self, qtbot, db) -> None:
        w = self._make_window(qtbot, db)
        rig, mock_sock = self._make_net_rig()
        rig.is_sdr = True  # type: ignore[attr-defined]
        w._apply_dial_feedback(rig, 145_800_000.0)
        assert w._dial_feedback_offset_hz == 0.0
        mock_sock.sendall.assert_not_called()

    def test_commanded_none_is_a_no_op(self, qtbot, db) -> None:
        w = self._make_window(qtbot, db)
        rig, mock_sock = self._make_net_rig()
        w._apply_dial_feedback(rig, None)
        assert w._dial_feedback_offset_hz == 0.0
        mock_sock.sendall.assert_not_called()

    def test_below_threshold_no_change(self, qtbot, db) -> None:
        w = self._make_window(qtbot, db)
        rig, mock_sock = self._make_net_rig()
        mock_sock.recv.return_value = b"145800000\n"  # 0 Hz off commanded
        w._apply_dial_feedback(rig, 145_800_000.0)
        assert w._dial_feedback_offset_hz == 0.0

    def test_manual_retune_accumulates_offset(self, qtbot, db) -> None:
        w = self._make_window(qtbot, db)
        rig, mock_sock = self._make_net_rig()
        mock_sock.recv.return_value = b"145809000\n"  # +9 kHz off commanded
        w._apply_dial_feedback(rig, 145_800_000.0)
        assert w._dial_feedback_offset_hz == 9_000.0

    def test_offset_accumulates_across_multiple_retunes(self, qtbot, db) -> None:
        w = self._make_window(qtbot, db)
        rig, mock_sock = self._make_net_rig()
        mock_sock.recv.return_value = b"145809000\n"
        w._apply_dial_feedback(rig, 145_800_000.0)
        mock_sock.recv.return_value = b"145805000\n"  # user nudges back down 4 kHz
        w._apply_dial_feedback(rig, 145_809_000.0)
        assert w._dial_feedback_offset_hz == 5_000.0

    def test_implausible_readback_ignored(self, qtbot, db) -> None:
        w = self._make_window(qtbot, db)
        rig, mock_sock = self._make_net_rig()
        mock_sock.recv.return_value = b"200000000\n"  # wildly off — comms glitch, not a retune
        w._apply_dial_feedback(rig, 145_800_000.0)
        assert w._dial_feedback_offset_hz == 0.0

    def test_get_frequency_failure_sentinel_ignored(self, qtbot, db) -> None:
        w = self._make_window(qtbot, db)
        rig, mock_sock = self._make_net_rig()
        mock_sock.recv.return_value = b"not_a_number\n"  # get_frequency() -> -1.0
        w._apply_dial_feedback(rig, 145_800_000.0)
        assert w._dial_feedback_offset_hz == 0.0

    def test_get_frequency_exception_swallowed(self, qtbot, db) -> None:
        w = self._make_window(qtbot, db)
        rig, mock_sock = self._make_net_rig()
        mock_sock.recv.side_effect = OSError("socket died")
        w._apply_dial_feedback(rig, 145_800_000.0)  # must not raise
        assert w._dial_feedback_offset_hz == 0.0

    def test_lock_toggle_resets_offset(self, qtbot, db) -> None:
        w = self._make_window(qtbot, db)
        w._dial_feedback_offset_hz = 9_000.0
        w._on_lock_changed(True)
        assert w._dial_feedback_offset_hz == 0.0
        w._dial_feedback_offset_hz = 9_000.0
        w._on_lock_changed(False)
    ...
    def test_doppler_cycle_folds_offset_into_dl_corr_when_locked(self, qtbot, db) -> None:
        """With Lock on and a nonzero accumulated offset, _doppler_cycle()
        must add it to dl_corr (and, via the existing Lock branch, to the
        computed ul_corr too) rather than only using it for detection."""
        ...
        class _FakeEngine:
            def observe(self, norad: int) -> Observation:
                return Observation(
                    ...
                    azimuth_deg=180.0,
                    range_km=1000.0,
                    range_rate_km_s=0.0,  # no Doppler shift — isolates the offset math
        ...
        class _FakeRig:
            ...
            def set_vfo_frequencies(self, dl: float | None, ul: float | None) -> None:
                self.calls.append((dl, ul))
        ...
        w._dial_feedback_offset_hz = 9_000.0
        rig = _FakeRig()
        w._rig_controller = rig  # type: ignore[assignment]
        w._doppler_cycle()
        qtbot.waitUntil(lambda: len(rig.calls) > 0, timeout=2000)
        dl, ul = rig.calls[0]
        assert dl == 145_800_000.0 + 9_000.0
        assert ul == 435_000_000.0 + 9_000.0  # Lock mirrors the same raw delta

    def test_doppler_cycle_two_cycles_detects_manual_retune_net_mode(self, qtbot, db) -> None:
        """End-to-end through a real HamlibNetController + a scripted fake
        socket, across three _doppler_cycle() calls:

        Cycle 1: no _last_commanded_dl_rig1_hz yet, so no read-back check
        runs; the write establishes a baseline.
        Cycle 2 (after the fake socket is reconfigured to report a different
        downlink, simulating a manual VFO retune between cycles): the
        pre-write read-back detects the mismatch and accumulates the offset
        — but this cycle's own write still uses the *stale* offset, since
        dl/ul were already computed by the outer synchronous code before
        this background thread ran. This 1-cycle lag is the accepted
        trade-off for never blocking DopplerWorker's own thread on a CAT
        round-trip.
        Cycle 3: the freshly-detected offset is finally folded into dl_corr
        and written.
        """
        from datetime import UTC, datetime
        ...
        class _ScriptedSocket:
            """Replies RPRT 0 to any write command, and to 'f' replies with
            whatever freq_reply currently holds (settable mid-test to
            simulate the rig's frequency changing between cycles)."""

            def __init__(self) -> None:
                self.last_cmd = b""
                self.freq_reply = b"145800000\n"

            def sendall(self, data: bytes) -> None:
                self.last_cmd = data

            def recv(self, _n: int) -> bytes:
                if self.last_cmd.strip() == b"f":
                    return self.freq_reply
                return b"RPRT 0\n"
        ...
        # Cycle 1: baseline write; nothing to compare against yet.
        w._doppler_cycle()
        qtbot.waitUntil(lambda: w._last_commanded_dl_rig1_hz is not None, timeout=2000)
        assert w._last_commanded_dl_rig1_hz == 145_800_000.0
        assert w._dial_feedback_offset_hz == 0.0

        # Simulate a manual retune to 145.809 MHz (+9 kHz) between cycles.
        sock.freq_reply = b"145809000\n"

        # Cycle 2: pre-write read-back detects it; this cycle's own write
        # still goes out un-offset (computed before the detection ran).
        w._doppler_cycle()
        qtbot.waitUntil(lambda: w._dial_feedback_offset_hz != 0.0, timeout=2000)
        assert w._dial_feedback_offset_hz == 9_000.0
        assert w._last_commanded_dl_rig1_hz == 145_800_000.0

        # No further manual intervention: the rig now reflects cycle 2's
        # (still un-offset) write.
        sock.freq_reply = b"145800000\n"

        # Cycle 3: read-back matches _last_commanded_dl_rig1_hz (no new
        # detection), and this cycle's write finally carries the offset.
        w._doppler_cycle()
        qtbot.waitUntil(lambda: w._last_commanded_dl_rig1_hz == 145_809_000.0, timeout=2000)
        assert w._dial_feedback_offset_hz == 9_000.0
