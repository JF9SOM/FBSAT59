...
xgettext --language=Python --keyword=_ --keyword=_N --keyword=ngettext:1,2 \
    --from-code=UTF-8 \
...
### 2026-09-19 の .po マージ後の状態（メモ）

Telemetryタブの「デコードに必要な最低SNR」ウィンドウ追加に伴い `xgettext`→`msgmerge` を実行した。
今回追加した文字列は翻訳済み。あわせて、他機能の追加で古くなっていた fuzzy 5件（`1 s`・`2 s`・
`5 s`・`0.5 s`・`Rotator update interval`。いずれも機械の推測が誤訳だった）を直して fuzzy を0にした。
**未翻訳は57件**（`msgfmt --statistics`。ヘッダーの空エントリを除く実質56件）。
**すべて意図的に英語のまま残す（2026-09-19 ユーザー判断: 翻訳不要、当面は英語のままにする）**:
Dashboard・Pass Chart 等のタブ名や CQ/RST/RR73 等の手続き語のほか、別機能で追加された文字列
（METEOR の Cities Overlay・SDR Waterfall・FT4 Waterfall・TX watchdog・TLE 取得失敗メッセージ等）。
`msgmerge` 後にこれらが未翻訳として現れても、**訳す提案をしないこと**（ユーザーが必要と言った
場合のみ訳す）。fuzzy は従来どおり0に保つ。
