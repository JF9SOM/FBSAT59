...
**未翻訳は57件**（`msgfmt --statistics`）で、うち約24件は従来どおり意図的に英語のまま
（Dashboard・Pass Chart 等）。残りの約30件は別機能（Cities Overlay・FT4 Waterfall・
SDR Waterfall・TX watchdog 等）で追加された新しい文字列で、まだ訳していない。
