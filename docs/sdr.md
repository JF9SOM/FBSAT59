...
- バーストの**左端に出現時刻**（`471.2 s` のように秒・小数1桁。S/N と同じ 16 pt 太字）。
  「0 秒」の基準は `SDRPipeline._burst_time_reference()` が決める:
  **IQ 再生中はファイル先頭からの位置**（＝録音ファイル内の時刻。解析で使う「t=471 s」と同じ値）、
  **ライブ受信で IQ 録音中は録音開始からの経過**（書き込み中のファイル内の時刻。録音器へ渡した
  サンプル数から数えるので、録音側の取りこぼしがあるとファイル内時刻より少し進んで見える）、
  **録音なしのライブは Burst 検出の開始（チェック ON）から**（録音とは無関係の代用基準）。
