...
### Pass Overview（IQ 録音の全体スペクトログラム、2026-09-26 追加）

SDR Control タブ再生行の「📂 Open…」で録音を開くと、再生開始と同時に全体像が別ウィンドウに自動表示される
（独立したボタンは無い）。`.iq.wav` を1回だけ読み、時間×周波数の全体像を別ウィンドウ（`src/ui/sdr_overview_dialog.py`）に表示する。
計算は `src/sdr/overview.py`（numpy のみ・scipy 不使用。CI に scipy が無いため）。

- 2048 点 FFT を 0.1 秒ごと（最大 2000 行に粗化）に取り、1024 列に平均。`np.memmap` で読むので 1.4 GB でも数秒。
- WAV ヘッダは手動解析し、data サイズはヘッダ値ではなく実ファイルサイズを使う（録音中はヘッダが古いことがあるため）。
- 「Remove background」（既定 ON）は周波数列ごとの時間方向中央値を引く。常時出ている線は消え、バーストは残る。録音全体でほぼ常時出続ける信号や、ゆっくり動く線は薄くなる。
- 色範囲はパーセンタイルで自動決定。Save PNG で軸付き画像を保存できる。
- 画像を左クリックするとその時刻へ再生位置がジャンプする（`seek_requested` → `SdrControlWidget._on_overview_seek()` → `SdrFileDevice.seek()`）。ホバーで時刻・オフセットを表示。別の録音を開くと前の全体像ウィンドウは閉じる。再生が終端まで進んで停止している状態でクリックした場合は、ジャンプ後に自動で再生を再開する（手動で Stop した場合は再開しない）。
