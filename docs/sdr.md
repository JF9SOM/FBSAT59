...
#### 未対応の発見: `Demodulator` の出力レートが 48 kHz ではない

上の検証中に判明。`sdr/demodulator.py` の各モードは整数の間引きで音声を作るため、
`AUDIO_RATE`(48 kHz)と表示しつつ**実際の出力レートはずれている**（16384サンプルブロックを
入力して実測）:
...
影響: スピーカー再生の音程がずれる（250 kS/sのUSB/CWで約23%低い）ほか、`audio_ready`を
受け取る CW Decoder・FT4・Q65・SSTV の各タブも実際とは違うレートのPCMを48 kHzとして
扱っている可能性がある。また音声を48 kHzで再生する側より速く供給されるため、
上記の別スレッド再生ではキューが溢れて定期的に音が途切れる（SDRのIQには影響しない）。
`G3ruhDiscriminator`と同様に`math.gcd`の有理数リサンプルで厳密に48 kHzへ揃えるのが
本筋だが、**未実施**（各デコーダータブへの影響確認が必要）。
