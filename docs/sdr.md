...
- 画像を左クリックするとその時刻へ再生位置がジャンプする（`seek_requested` → `SdrControlWidget._on_overview_seek()` → `SdrFileDevice.seek()`）。ホバーで時刻・オフセットを表示。別の録音を開くと前の全体像ウィンドウは閉じる。クリックは常に「ジャンプして再生」する（再生中・Stop 後・終端後のいずれも。位置だけ動かしたい場合は下のスライダーを使う）。
