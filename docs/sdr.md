...
**今回は触っていない既存の問題（観察）**: (1) SSBの BFO ミキシングは各ブロックで
位相が0に戻る（`t = arange(n)/rate`）ため、ブロック境界ごとに位相の不連続があり、
USB/LSBの音がわずかにずれて聞こえる（250 kS/sのUSB 2000 Hz搬送波で理想650 Hzに対し
657 Hz、2.4 MS/sでは682 Hz）。(2) NFM のIF・デエンファシス、SSBのLPF、CWのBPFは
ブロックごとに状態が初期化される（G3RUH弁別器で直したものと同種）。どちらも音声の
ブロック境界に小さな乱れを残す。
