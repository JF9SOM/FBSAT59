...
Baudを「自動」のままにしておくと常に1200bps用の`AfskDemodulator`が起動してしまい、
9600bpsのG3RUH信号は原理的にデコードできない。**Baudを手動で「9600」（または該当する
レート）に選択する必要がある。**
...
`AfskDemodulator`と`G3ruhSdrDemod`を短時間並行起動して信号の有無で判定する、等の
設計が考えられる。
