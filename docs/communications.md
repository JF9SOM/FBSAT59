...
- **Show トグル（平文 ⇔ 生パケット、2026-09-08 実装）**: 局設定行の Baud の右。
  - **平文**（既定）: `src/comms/aprs/humanize.py`（`aprslib` ベース）が MIC-E・圧縮位置・
    ...
    位置には**最寄り都市**（`src/comms/aprs/citylookup.py` ＋ 同梱 GeoNames 抽出
    `src/data/cities15000.tsv.gz`、population≥15000、CC BY 4.0、完全オフライン、
    50km 超は付けない）を「◯◯付近 / near ◯◯」で付加。
...
|---|---|---|---|---|
| SSTV | FM 音声変調（アナログ） | sounddevice（44.1 kHz）/ SDR 音声出力（48 kHz） | **自前デコーダー**（`comms/sstv/decoder.py`、numpy/scipy。pySSTV は使っていない） | ISS（Robot36/PD120）・IO-86 等 |
| SSDV（音声経路） | FM 音声変調（デジタルパケット） | sounddevice / SDR 音声出力 | `ssdv` CLI ツール（サブプロセス） | 一部 CubeSat |
| SSDV（AX.25経路） | AX.25 パケット | SSDV タブが自分で AX.25 受信を開始（SDR / Rig+サウンドカード）。**HEX 貼り付けでも可** | `ssdv` CLI ツール | JY1Sat 等（将来） |
...
│   ├── sstv/
│   │   ├── __init__.py
│   │   ├── decoder.py      # SstvDecoder — 自前のストリーミングデコーダー（Robot36/PD120）
...
- **実際の音声再生はしない**（無音・高速デコード）。ファイル全体を`SstvDecoder.push_samples()`に
  **一度だけ**渡す。ライブ経路のように数秒ごとのチャンクに分けて渡すと、`SstvDecoder._process()`
  ...
  （2026-09-24 のデコーダー作り直しで、**1 つの録音に複数の SSTV 送信があっても順に全て**デコードできる。
  録音が画像の途中で終わっていても、`stop()` が届いた分を画像として出す）
...
#### SSDV の受信・「生パケット」/「画像」サブタブ（2026-09-24 実装）

**それまで**: SSDV モードは他のタブが起こした AX.25 受信の生フレームを覗き見るだけで、APRS タブが
開いていないと「先に APRS タブを開いてください」と出た（`MainWindow` が APRS タブの `engine` を
探して渡す作りだった）。しかも判定が「フレームの先頭が `55 66`」だったため、AX.25 ヘッダ付きの実際の
フレームは**一度も一致しなかった**。

**今**:

- **自分で受信を開始**: SSDV モードにすると `AprsEngine`（シングルトン）を owner `"sstv"` で
  起動する（SDR → `start_sdr_direwolf(..., satellite=True)`、Rig+サウンドカード → `start_rig`、
  ボーは `resolve_ax25_modem`）。入力の接続・切断、トランスポンダー変更（ボー変更）にも追従する
  （Telemetry タブと同じ方式）。入力が無くても HEX 貼り付けは使える。`MainWindow` の APRS タブ探索は削除。
- **画像表示区域が 2 つのサブタブ**: 「生パケット」（`_RawPacketEdit`）と「画像」。SSTV モードは画像、
  SSDV モードは生パケットを自動選択。
- **生パケット**: 受信フレームを **1 行 1 フレームの HEX**（`94 A6 62 …`、Telemetry タブ・SatNOGS と同形式）で
  リアルタイム追記（最大 5000 行）。カウンタは「フレーム数 / SSDVパケット数」。
- **貼り付けで画像生成**: テキスト欄にテレメトリータブの HEX や SatNOGS Network の「Data」タブの HEX を
  貼ると、**欄の全文**を解析して画像を作り、「画像」タブへ切り替える（「HEXから画像を生成」ボタンでも同じ。
  「パケットを消去」で全消去）。貼り付けは必ず**独立した行**として入る（カーソル行にくっつけない。
  最初の実装は貼り付け位置の行に連結して壊れた）。`extract_hex_frames()` は、HEX バイトだけの行を
  フレームとみなし、`94 A6`・`94a6`・`0x94,0xA6`・`94:A6` を受け付け、表のコピー（タブ区切り）は最後の
  セルを使い、SatNOGS のタイトル行（`data_obs/2026/9/23/...`）や空行、時刻などは無視する。
- **SSDV パケットの検出**（`find_ssdv_packet()`）: フレーム中の任意の位置の `55 66`（FEC あり）/ `55 67`
  （FEC なし）から末尾（最大 256 バイト）までを候補とし、**CRC-32 が合うものを採用**（AX.25 ヘッダや
  KISS バイトが前にあってよい）。CRC が合わなくても 256 バイトの `55 66` は `ssdv` の Reed-Solomon 訂正に
  任せて渡す。`ssdv -l` の**短いパケット長**（21〜256 バイト）にも対応し、最頻の長さで `-l` を指定する。
- **`SsdvDecoder`**: 画像 ID ごとに整理し、重複を除き、パケット ID 順に並べ、**最もパケットの多い画像**を
  `ssdv -d` に渡す。受信中は 700 ms ごと（バースト単位）に再デコードして画像が育っていく。
...
（`tests/test_ssdv.py`、`ssdv` が無い環境ではその 4 件は skip）。UI は `tests/test_sstv_tab.py`。
**OrigamiSat-2 のフレームには SSDV パケットが見つからなかった**（`55 66`/`55 67` 無し。約 50 バイトの
別形式）。ARICA-2 の 100 バイトフレームも標準の SSDV ではなさそう。標準外の形式は未対応。

#### SSTV デコーダーの作り直しと起動バグの修正（2026-09-24）

**判明したこと（それまで SSTV タブは実質的に動いていなかった）**:

1. **起動バグ**: タブを開くと「SSTV」と表示されるのに、デコーダーは**起動していなかった**（`_start_decoder()` は
   モードを切り替えたときだけ呼ばれ、SDR/無線機の接続でも、デコーダーが無いと音声を接続しなかった）。
   「SSDV」に切り替えて「SSTV」に戻して初めて動いた。今はタブを開いた時点で起動する。
2. **サンプルレートの取り違え**: SDR の音声は 48 kHz（`sdr.demodulator.AUDIO_RATE`）なのに、デコーダーは常に
   44.1 kHz として処理していた（全ての周波数・時間が 8.8% ずれ、何もデコードできない）。今は入力源に合わせる
   （SDR 48 kHz / サウンドカード 44.1 kHz。入力が変わると `set_sample_rate()` で再設定）。
3. **デコーダー自体が動くはずのない実装だった**: 音声を 4 秒ごとの塊に分けて塊ごとに画像を最初から作り直す
   （Robot36 は 1 枚 36 秒なので画像が完成しない）／PD120 のクロマの長さ（121.6 ms のところ 60.8 ms）と順序（R-Y→B-Y）が
   誤り／Robot36 のクロマ合成が誤り／VIS 検出も傾き補正も無し。**実信号でも合成信号でも動作確認されたことが
   一度もなかった**。ドキュメントの「pySSTV ラッパー」は事実誤認（pySSTV は**エンコーダーのみ**でデコーダーは無い。
   アプリには使えないが、**独立した基準信号の生成**に最適なので検証に使った）。

**新しいデコーダー**（`comms/sstv/decoder.py`、Robot36 / PD120）:

- ストリーミング（任意サイズのチャンクを受け、1 行ずつ画像を育てる）。周波数は 1700 Hz へ混合→FIR ローパス
  （1000 Hz）→FM 弁別（フィルタ状態を持ち越すのでチャンク境界が見えない）。ローパスは狭いほど雑音に強い
  （12 dB で 1300 Hz より誤差 25% 減、2200 Hz より 40% 減）が、無雑音での鮮鋭さはわずかに落ちる。
- 開始: **VIS ヘッダー**（リーダー 1900 Hz 300 ms／ブレーク／リーダー／開始ビット／7 ビット＋偶数パリティ／
  停止ビット）でモードを判定。VIS を取り逃した場合（弱信号・画像の途中から始まる録音）は、**1 行周期で
  等間隔に並ぶ同期パルス 5 本**で開始する。
- 各行: 同期パルス（1200 Hz）を予測位置の近くで探して**行ごとに再同期**（サウンドカードのクロック誤差による
  傾きを補正。±1000 ppm で確認）。ピクセルは**各ピクセル区間の平均周波数**（1 点サンプリングより雑音に強い）。
  **同期の位置は「終わりの端」から出す**: 立ち上がりの端は前の行の末尾の周波数（1500〜2300 Hz）次第でしきい値の
  横切りが遅れ、画像が約 2 ピクセル左にずれていた（終わりの端は 1200→1500 Hz の中点で偏らない）。
- 色: JFIF YCbCr（pySSTV と同じ）。Robot36 は偶数行 R-Y・奇数行 B-Y（2 行で 1 組）、PD120 は Y・R-Y・B-Y・Y。
- 終了: 最終行／同期が 8 行連続で無い（信号消失。ゴミ行は黒く消して、届いた分を画像として出す）／`stop()`。
  5 行未満は画像として出さない。

**検証**（`tests/test_sstv_decoder.py`、pySSTV の音声で 29 項目）: Robot36・PD120 × 44.1/48 kHz で 1 枚・正しい
モード・サイズ・PSNR（原画像比 Robot36 約 29 dB／PD120 約 34 dB。色の解像度が半分のため 30 dB 台が上限）、位置ずれ
なし、チャンクサイズ非依存（1000〜1,000,000）、雑音 12 dB でも復元、クロック誤差 ±0.1%、40 Hz の同調ずれ、
ヘッダー前の雑音、2 枚連続、VIS なし（行 30 から始まる録音）、途中で終わる/信号が消える、無音・雑音のみで誤検出なし、
サンプルレートの取り違え。**実際の衛星・ISS の SSTV 信号での検証はまだ**（録音が無い）。
...
#### SDR 純 Python 復調パイプライン（受信専用・2026-09-13 に廃止・Direwolf経由方式へ全面置換）

**この節はv0.2.0時点の当初設計であり、現在は採用していない。** 当初はBell 202 AFSK
（1200 baud）をnumpy + scipyで自前実装（このバンドパス＋mark/space電力比較という設計
方針自体は最終的に`afsk_demod.py`の`_process()`にほぼそのまま実装された）していたが、
実信号で最後まで安定デコードできず、2026-09-13に**SDR側もDirewolf本家のMODEM 1200
デコーダーに任せる方式**へ全面的に置き換えた。経緯・診断の全過程は後述
「1200bps SDR単体経路（`AfskDemodulator`）の実装不備の発見・修正」以下の一連の節、
最終的な解決は「1200bpsもDirewolf経由方式へ全面移行（2026-09-13、実受信で解決確認済み）」
節を参照。`afsk_demod.py`自体は削除済み。
...
#### SDR単体経由（9600bps）実信号での検証完了（2026-09-12）

地上系JAPRSデジピーター網（144.64MHz、9600bps）での実受信IQをRTL-SDR経由で録音し、
`G3ruhDiscriminator`→Direwolf本家G3RUHデコーダーの経路をオフラインで検証したところ、
以下のサンプルレート不一致バグが見つかった:

`G3ruhDiscriminator._build_filters()`の最終間引き段が単純な整数ストライド
（`mid_rate // int(mid_rate / 48000)`）だったため、**現実的などのSDRサンプルレート
（250kHz・1.024/2.4/3.2Msps）でも実際の出力レートは48000Hzちょうどにならず、
常に約50000〜51200Hzになっていた**（`200000/48000=4.1666...`が整数除算で`4`に丸まる
ため、`_INTERMEDIATE_RATE_TARGET=200_000`という定数自体が48000の整数倍でないことに
起因する、入力レートに依存しない恒常的なバグ）。一方`direwolf.py`の`_write_config()`は
`ARATE 48000`を無条件宣言するため、Direwolf自身のG3RUHビットクロック同期に対して
常に4〜6.7%のレート偽り（宣言と実際の音声クロックの食い違い）を与え続けていたことになる。
`g3ruh_demod.py`冒頭のdocstring通り「実際の9600bps G3RUH衛星信号での検証を一度も
行っていない」実装だったため、恐らく一度も正常動作したことがなかったと考えられる。

**修正**: 最終段を`scipy.signal.resample_poly`による厳密な有理数リサンプル
（`math.gcd(48000, mid_rate)`でup/downを求める）に置き換え、宣言レートと実レートを
必ず一致させた。実際に録音したIQを本修正の`G3ruhDiscriminator`に通し、Direwolf本家
（`-B 9600`・`ADEVICE stdin stdout`、アプリと同一構成）にオフラインで流し込んだところ、
修正前は0件、**修正後は実際のAX.25フレームを複数正常デコード**（JAPRS経由の
APRS-ISサードパーティゲートウェイパケット）することを確認した。

`_IF_HALF_BW_HZ`・`_DEVIATION_HZ`（NFM音声用パラメータの流用値）は今回のテストで
特に問題にならなかったため未変更。Rig + サウンドカード経由（外付けG3RUH対応
サウンドカード等の実ハードウェア）でのMODEM 9600自体の実衛星パスでの動作確認は
引き続き未実施。

#### 9600bps G3RUH の感度改善 — IF 帯域の適正化とブロック境界バグの修正（2026-09-19）

KNACKSAT-2（NORAD 67683、400.630 MHz、9600bps G3RUH）の実パスをIQ録音し、再生
（`docs/sdr.md`「IQ録音の再生機能」）してデコードを試みたが1フレームも出なかった。
調査の結果、録音のテレメトリ・バーストは弱すぎて**この録音自体は復号不能**（後述）
だったが、その過程で`G3ruhDiscriminator`に感度を大きく損なう欠陥が2つ見つかり修正した。

**検証方法（重要）**: 9600bps G3RUH AX.25 フレーム（Gaussian BT=0.5、偏移±3 kHz、
約170 ms/フレーム、SDRパイプラインと同じ16384サンプルブロック単位）を合成し、
雑音を加えてアプリの`G3ruhDiscriminator`→Direwolf 本家（`MODEM 9600`、アプリと同一構成）
に流し、**実際にデコードできたフレーム数**（40フレーム中）で比較した。SNRは
「信号電力÷11 kHz帯域の雑音電力」。理想波形（FM無し）を直接Direwolfに流して20/20
デコードできることで、生成器・ハーネス自体の正しさを先に確認している。

1. **IF フィルタがブロックごとに状態を初期化していた（バグ）**。`process()`は16384
   サンプル（約65 ms）ごとに独立に呼ばれるが、IFフィルタ（`lfilter`）が`zi`を引き継がず
   毎ブロック先頭で起動時の過渡応答（63タップ分）が入っていた。1バースト（約170 ms）に
   2〜3回の境界が入るため、**雑音がほぼ無い40 dBでも合成9600bpsフレームが8/40しか
   デコードできなかった**（状態を引き継ぐだけで40/40）。長いフレームほど失敗する。
   → IF・アンチエイリアス・弁別後LPFの各`lfilter`と、弁別器の直前サンプル、間引き位相を
   全て呼び出し間で引き継ぐように変更。
2. **IF が広すぎた（±13 kHz）**。9600bpsの信号は約±6 kHzに収まるが、FM弁別器の出力雑音は
   IF帯域が信号より広いほど急増する。合成信号で測定（255タップ・状態引き継ぎ済み）:

   | SNR (11 kHz) | ±13 kHz | ±8.5 kHz | ±7 kHz | ±6 kHz | ±5 kHz |
   |---|---|---|---|---|---|
   | 10 dB | 0 | 0 | 5 | 21 | 16 |
   | 12 dB | 0 | 14 | 37 | 39 | 36 |
   | 14 dB | 2 | 36 | 40 | 40 | 40 |
   | 16 dB | 35 | 40 | 40 | 40 | 40 |

   狭いほど感度は上がるが周波数誤差（衛星の発振器ずれ・Offset設定ずれ）への許容度が
   下がる（±7 kHzで±2 kHzずれるとSNR 12 dB付近で25〜30/40に低下）。許容度も測って
   **IF半帯域 7.5 kHz + 弁別後LPF 6.5 kHz（101タップ）**を採用（±2 kHzずれでもSNR
   11.5 dBで32〜38/40、13.5 dBで40/40）。弁別器出力のフルスケールは、周波数誤差による
   DCオフセットでクリップしないよう5 kHz→8 kHzに拡大（Direwolfの9600 demodは振幅・
   DCに自動追従するため音量低下の影響は測定上なし）。

   `_Profile`テーブル（`G3ruhDiscriminator(baud=...)`）でボーレートごとに設定する。9600は
   上記。**4800も同日中に追加で狭めた**: 合成4800bps（偏移±1.5 / ±2.4 kHzの両方）で
   IF半帯域・弁別後LPFを格子探索し（IF 3〜5.5 kHz × LPF なし/3.5/4.5 kHz）、周波数誤差
   許容度も測って**IF半帯域4.5 kHz + 弁別後LPF 3.5 kHz + フルスケール8 kHz**を採用した。
   IFを3.5〜4 kHzまで狭めるとSNR 11 dB付近の感度はさらに少し上がるが、±1.5 kHzの周波数
   ずれでほぼ復号できなくなるため、許容度を優先した。プロファイルの無いボーレートは
   従来のIF（±13 kHz・63タップ）のまま。

   （なお、この時点で4800bpsが実信号で復号に成功した記録は無い。ARICA-2の検証は
   `ARATE`必須の発見までで、フレーム復号成功の記録ではない。4800の設定は合成信号の
   測定のみに基づく。）

**アプリの`G3ruhDiscriminator`そのものでの結果**（250 kHz入力、16384ブロック、
デコード数/40。Old=修正前、New=修正後）:

| SNR (11 kHz) | 40 | 20 | 16 | 14 | 13 | 12 | 11 |
|---|---|---|---|---|---|---|---|
| Old | 8 | 10 | 7 | 1 | 0 | 0 | 0 |
| New | 40 | 40 | 40 | 40 | 40 | 39 | 36 |

SDRサンプルレート1.0 / 2.4 Msps（間引き経路）・SNR 14 dBでも Old 3/0 → New 40/40。

**4800bpsの結果**（アプリの`G3ruhDiscriminator`、SNRは5.5 kHz帯域、デコード数/40。
Old=修正前、New=修正後）:

| 偏移 | SNR | 20 dB | 16 | 14 | 13 | 12 | 11 |
|---|---|---|---|---|---|---|---|
| ±1.5 kHz | Old | 11 | 2 | 0 | 0 | 0 | 0 |
| | New | 40 | 40 | 40 | 40 | 32 | 15 |
| ±2.4 kHz | Old | 18 | 18 | 4 | 0 | 0 | 0 |
| | New | 40 | 40 | 40 | 39 | 38 | 22 |

周波数誤差（SNR 16 dB）は±1.5 kHzまで復号を維持（偏移±2.4 kHzで40/40、±1.5 kHzで
35〜40/40）、±2.5 kHzでは復号できない（Oldは低SNRで不安定ながら±2.5 kHzでも一部復号）。
SDRサンプルレート1.0 / 2.4 Msps・SNR 16 dBでは Old 25/0 → New 40/40。

つまり実効感度が約16 dB → 約11 dB（5 dB以上）改善し、雑音の少ない信号でも長いフレーム
が落ちる問題が解消した。実信号（JAPRS 9600bps）での再確認は未実施。

**1200bps（Bell 202 AFSK）も同日中に同じ手当てをした**。`AfskAudioDiscriminator`
（`afsk_audio_demod.py`）は`G3ruhDiscriminator`と全く同じ構造でIFフィルタの状態を引き継いで
いなかったため、`G3ruhDiscriminator`を継承して**共通の処理を使う**形に整理した
（`DiscriminatorProfile`に`deemph_tau_s`を追加し、AFSK側は75 µsのデエンファシスを持つ
プロファイルを渡すだけ）。Bell 202フレーム（約0.8 s、Gaussian無し・トーン1200/2200 Hz、
FM偏移±3 / ±5 kHz、16384ブロック）を合成してDirewolf（`MODEM 1200`）で復号し比較した。
1200bpsは9600bpsと違って、強い信号ではブロック境界の乱れがあってもDirewolfが復号でき
（修正前でも40 dBで30/30）、差は低SNR側に出る。IF半帯域の探索（255タップ）:

| 偏移 | SNR (10 kHz帯域) | ±4 kHz | ±5 | ±6 | ±7.5 | ±9 | ±13 |
|---|---|---|---|---|---|---|---|
| ±3 kHz | 9 dB | 4 | 11 | 15 | 2 | 0 | 0 |
| ±5 kHz | 9 dB | 0 | 28 | 30 | 29 | 29 | 10 |

（デコード数/30。旧IFは±9 kHz・63タップで、フィルタのスカートはさらに広かった。）
周波数ずれ許容度も測って**IF半帯域7 kHz**を採用した（±6 kHzは最も高感度だが、偏移±5 kHzで
2 kHzずれると復号できなくなる。±7 kHzは両偏移で±2 kHzずれを許容し、旧±9 kHzより約2 dB
高感度）。弁別後ローパスと出力フルスケールの拡大は1200bpsでは効果が無かったため入れて
いない。デコード数/30（アプリのクラス、修正前=Old / 修正後=New）:

| 偏移 | SNR (10 kHz) | 20 dB | 14 | 12 | 10 | 9 | 8 |
|---|---|---|---|---|---|---|---|
| ±3 kHz | Old | 30 | 24 | 18 | 1 | 0 | 0 |
| | New | 30 | 30 | 30 | 17 | 3 | 0 |
| ±5 kHz | Old | 30 | 29 | 30 | 28 | 28 | 21 |
| | New | 30 | 30 | 30 | 30 | 30 | 28 |

SDRサンプルレート1.0 / 2.4 Msps（間引き経路）・SNR 12 dBでは Old 7/0 → New 30/30と特に
大きく改善する（旧実装は間引き位相の不連続も重なっていた）。ライブの受信で1200bpsが
強い信号でしか復号できなかったことと整合するが、実信号での再確認は未実施。テストは
`tests/test_afsk_audio_demod.py`。

**未着手の観察（参考）**: 3つのボーレート共通のI/Q直流除去（30 Hz、RTL-SDRのLO直流
スパイク対策）は、中心に同調した狭帯域FMの搬送波成分も削る。直流除去を弱めた場合の
合成信号での効果は、1200bpsでSNR 9〜10 dBの復号数が4→12、18→27/30（約1 dB相当）、
9600bpsではほぼ無し。ただし実SDRの直流スパイクが復号を妨げる効果とのトレードオフ
（実信号なしでは判断不能）のため変更していない。

**KNACKSAT-2 の実録音（2026-09-19）の結論**:
- 録音中の強く見える細い線（ドップラー補正基準から+約19.5 kHz）はテレメトリではない
  （データを含まない連続キャリア）。実テレメトリは公称周波数（0 kHz付近）に**30秒周期
  （録音の 16+30k 秒）・約185 ms・幅約11 kHz**のバーストとして現れる。SatNOGSの
  同一パス観測（局4394）が400.630 MHzで同時刻にフレームをデコードしている
- 自局でのバースト SNR は最大でも+4〜+8 dB（11 kHz帯域、録音256〜376秒。他の時間帯は
  雑音以下）。修正後の復号限界（約11〜12 dB）に**4〜8 dB足りず**、修正前・修正後とも
  Direwolf で0フレーム（雑音由来の偽候補1件のみ）。受信系（アンテナ・LNA）または高仰角
  パスの改善が必要
- Offset は0付近（バースト中心は約−0.5 kHz）。強い線に合わせて+20 kHzにしても復号
  できない

**Direwolf の設定ファイルパスの落とし穴（検証ハーネス作成時に発覚）**: Direwolfは
`-c`に渡す設定ファイルのパスが約100文字を超えると**黙って切り詰め**、設定を読めずに
デフォルト動作（stdinを読まない）へ落ちる。長いパスで一括テストすると「全ケース0フレーム」
という一見もっともらしい結果になる（当初の実録音検証がこれで無効だった）。カレントを
設定ファイルの場所に移して**相対パス**（短い名前）で渡すこと。アプリ本体は
`tempfile.mkstemp`の短いパスなので影響なし。

**テスト**: `tests/test_g3ruh_demod.py` — ブロック境界回帰（250 kHz・間引き経路、9600/4800）、
ブロックサイズ非依存、狭IF（9600・4800）が帯域外の強い干渉波を除去する（プロファイル無しの
レガシーIFは通す）こと、各ボーレートの信号帯域を正しく通すこと。ブロック単位のリサンプル（`resample_poly`）は
ブロック先頭の1〜2サンプルにごく小さな端点リップル（最大約0.1）を残すが、合成信号の
デコード結果（40/40）には影響しなかったため未変更。

#### 感度の再測定ツール `scripts/g3ruh_sensitivity.py` と GRBBeta 実録音の結論（2026-09-19）

上記の検証ハーネスは使い捨てだったため、同じ測定をいつでも再現できる開発用ツールとして
`scripts/g3ruh_sensitivity.py` を追加した（アプリ本体からは使われない）。同じ250 kHz IQを
**Direwolf経路**（アプリの`G3ruhDiscriminator`→Direwolf 本家）と**gr-satellites**
（`gr_satellites 60237 --rawfile … --iq`、Telemetryタブのgr-satellitesモードと同じ呼び出し）の
両方に流し、デコード数を比較する。

- `sweep`: 合成9600bps G3RUHフレームを雑音に埋めてSNR（11 kHz帯域）ごとに40フレームの
  復号数を表示。`--offset-hz`（周波数誤差）・`--preamble-flags`・`--decoders`で条件を変える。
  デモ復調チェーンの変更で感度が落ちていないかの確認や、復号限界の測定に使う
- `file`: 実録音（SDR ControlタブのIQ録音WAV、または生`.cf32`）を両経路に通す。
  `--start`/`--duration`（秒）で長い録音の一部だけを切り出せる
- Direwolfは`KISSPORT 0`/`AGWPORT 0`で起動するので、アプリ起動中でも実行できる

基準値（2026-09-19、合成信号）: 50%復号点は**Direwolf経路が約12.5 dB、gr-satellitesが約14 dB**
（11 kHz帯域SNR）。どちらも周波数誤差±2 kHzまで許容し、±4 kHzで失敗する。プリアンブル長
（20〜100 ms）は結果を変えない。gr-satellitesは生IQを自前の復調器で処理するため、
Direwolf経路のIF調整（上記）とは無関係で、追加のチューニングは不要。合成信号は理想的な
GFSKなので、実衛星ではこれより悪くなりうる（上限値として扱うこと）。

**GRBBeta（NORAD 60237、UHF 436.785 / VHF 145.935 MHz、9600bps G3RUH）の実録音
（2026-09-19 09:11:46 UTC、最大仰角51°）の結論**: 録音は前半VHF・後半UHF。
- VHF前半: ±12 kHzのどのオフセットにもバーストなし（検出下限はS/N約−5 dB）。
  ノイズフロアはUHFより約12 dB高く、広帯域インパルス雑音を含む
- UHF後半: 0 kHzに幅±4 kHz・約0.2秒のバーストが4本（09:18:04.4 / 09:18:06.8 /
  09:18:24.5 / 09:19:34.5）。11 kHz帯域S/Nは約−3.6 / −1.0 / −0.4 / +2.5 dBで、
  Direwolf経路の限界（約12.5 dB）に10〜16 dB足りず、Direwolf・gr-satellitesとも0フレーム。
  周波数ずれは−25〜+65 Hzで問題なし
- 同じパスのSatNOGS観測（別地点・最大仰角76°）ではUHFで6フレーム（09:10:51〜09:16:09）、
  VHFで1フレーム（09:21:36）。直近13時間ではVHF 3フレーム対UHF約70フレームで、
  **衛星は主にUHFで送信する**。UHFに固定して受信するのが有利
- 結論: ソフトウェア側の問題ではなく受信レベル不足。改善にはアンテナ・LNAなどの受信系
  （上記KNACKSAT-2の結論と同じ）

#### デコードに必要な最低SNRの目安（2026-09-19 整理。ⓘ ボタンは 2026-09-24 削除）

（この表は Telemetry タブの ⓘ ボタンのウィンドウの元データだったが、2026-09-24 に ⓘ ボタンごと削除した。
衛星系では実現困難な値で目安として不適切、というユーザー判断。測定記録として残す。）
数値は下記の**合成信号での測定**で、実衛星では変調・偏移・フレーム長が異なる目安。
**SNRは「信号電力÷その信号自身の帯域内の雑音電力」**（帯域: 9600 = 11 kHz、4800 = 5.5 kHz、
1200 = 10 kHz）。アプリはSNRを数値表示しないので、利用者はウォーターフォールで判断する。

**約90%のフレームがデコードできるSNR（dB）**:

| 速度 | Direwolf | gr-satellites（長い前置信号あり） | 信号帯域 | Direwolfの周波数許容差 |
|---|---|---|---|---|
| 1200 bps AFSK | ≈ 11 | ≈ 10 | 10 kHz | ±2 kHz |
| 4800 bps G3RUH | ≈ 13 | ≈ 16以上（16 dBで80%） | 5.5 kHz | ±1.5 kHz |
| 9600 bps G3RUH | ≈ 12 | ≈ 14 | 11 kHz | ±2 kHz |

**測定条件と生データ**（デコード数／N。Gaussian BT=0.5・偏移は下記・250 kS/s・16384ブロック）:

- **Direwolf**（本セッションで調整済みの `G3ruhDiscriminator` / `AfskAudioDiscriminator` 経由、
  `MODEM 9600` / `4800 G3RUH` / `1200`）
  - 9600（偏移±3 kHz、N=40）: 16 dB 40 / 14 dB 40 / 13 dB 40 / 12 dB 39 / 11 dB 36
  - 4800（N=40）: 偏移±1.5 kHz — 14 dB 40 / 13 dB 40 / 12 dB 32 / 11 dB 15、
    偏移±2.4 kHz — 14 dB 40 / 13 dB 39 / 12 dB 38 / 11 dB 22
  - 1200（N=30）: 偏移±3 kHz — 14 dB 30 / 12 dB 30 / 10 dB 17 / 9 dB 3、
    偏移±5 kHz — 10 dB 30 / 9 dB 30 / 8 dB 28
- **gr-satellites**（`gr_satellites <機種> --rawfile x.cf32 --iq --samp_rate 250000 --hexdump`、
  設定を変えていない現状のまま。9600=`ACRUX-1`、4800=`BOTAN`、1200 AFSK=`AmicalSat`、N=30、
  バースト先頭の前置信号0.5秒）
  - 9600: 16 dB 30 / 14 dB 28 / 13 dB 21 / 12 dB 7 / 11 dB 2
  - 4800: 16 dB 24 / 14 dB 15 / 13 dB 10 / 12 dB 4 / 11 dB 1
  - 1200 AFSK: 12 dB 30 / 11 dB 30 / 10 dB 28 / 9 dB 25 / 8 dB 3
  - 1200 AFSK ではDirewolfより約1〜2 dB感度が良く、4800/9600ではDirewolfより約2〜4 dB悪い

**gr-satellitesは前置信号（プリアンブル）が短いバーストを取りこぼす**（雑音がほぼ無くても）。
9600・`ACRUX-1`・N=30・バースト間300 ms の無音、前置信号の長さ別（16 / 20 / 30 dB）:
33 ms（フラグ40個、Direwolfの測定条件）14 / 12 / 17、166 ms 21 / 26 / 26、
498 ms 28 / 30 / 30、996 ms 28 / 30 / 30。同じ信号をバースト間の無音なしで連続送信すると
33 ms前置信号でも 12 dB 11 / 14 dB 23 / 16 dB 27 / 20 dB 28。**Direwolfは33 msの前置信号で
40/40**（帯域内の同期がゆるい）。つまり短いバースト（例: KNACKSAT-2の約185 msバースト全体）
にはDirewolfのほうが向く。gr-satellitesの周波数許容差は**未測定**。

**注意**: (1) 合成信号のみの測定で、実衛星・実SDRでの確認は未実施。(2) 上記はSNRが不足して
いない場合の話で、SDRのサンプル欠落（音声出力オン時の旧不具合、同日修正）があると
これらの値でも復号できない。(3) KNACKSAT-2の実録音（2026-09-19）のバースト SNR は
+4〜+8 dB（11 kHz帯域）で、どちらのデコーダーの限界にも届かなかった。

#### SDR 4800/9600bps のコヒーレント MSK 検波・1200bps 衛星用前段（2026-09-24 実装）

**きっかけ**: Telemetry タブで OrigamiSat-2（1200bps AFSK）と ARICA-2（4800bps GMSK）の
IQ 録音をどうしてもデコードできなかった。他局（北九州局の QFH など）は同じパスで
数十〜80 フレームを受信しており、内容も公開されている。以下は**その公開フレームと
自分のデコード結果を突き合わせて**確認した（ARICA-2 のバースト 1 は先頭 24 バイトが
完全一致）。調査の過程で「SNR が低いからデコードできない」という結論に何度も逃げたが、
実際は復調器の前段と復調方式に改善余地があった。

**原因と対策（要点）**

| 速度 | 原因 | 対策 |
|---|---|---|
| 1200bps | FM 弁別器の IF が広すぎ（±7 kHz）、低 SNR の FM 特有の**クリック雑音**が音声を壊す | `afsk_audio_demod._AFSK_SATELLITE_PROFILE`（下記）。Telemetry タブ専用 |
| 4800/9600bps | 弁別器＋Direwolf は約 12〜13.5 dB の Eb/N0 を要する。同じ信号を Direwolf / SatNOGS(gr-satnogs) / GNU Radio `gmsk_demod` / gr-satellites に通しても**どれも同じ数フレームで頭打ち**（＝復調器の実装差ではなく方式の限界） | **コヒーレント MSK 検波**（`comms/aprs/coherent_msk.py`）。約 4 dB 感度が上がる |

**1200bps の衛星用前段**（`AfskAudioDiscriminator(satellite=True)`、実録音 OrigamiSat-2
2026-09-23 で従来 0〜1 フレーム → 20〜21 フレーム、送信 24 フレーム中カウンタ 18〜19 個を回収）:
IF を ±5 kHz（従来 ±7 kHz）、弁別器を約 50 kHz で動かす（`disc_rate_hz`）、IF 包絡線が
10 ms 平均の 0.5 倍を下回る所で出力を絞る（`env_weight`、クリックは包絡線の谷で出る）、
出力を ±2.5 kHz でクランプ（`clamp_hz`）、ディエンファシスなし（ON/OFF で結果は同一だった）。
**クランプは偏移が小さいと分かっている衛星専用**（地上 APRS の ±5 kHz を歪ませる）ため、
Telemetry タブだけが `start_sdr_direwolf(..., satellite=True)` を渡し、APRS タブは従来のまま。
`DiscriminatorProfile` に `clamp_hz` / `env_weight` / `disc_rate_hz` を追加した（既定 None で
既存プロファイルの挙動は不変）。

**コヒーレント MSK 検波の仕組み**（変調指数 h=0.5＝偏移 baud/4 の GMSK/GFSK 専用）:
8 sps にリサンプル→LPF（4800: 3.5 kHz / 9600: 7 kHz）→シンボル境界のサンプルに
`(-j)^k` を掛けると BPSK になる→2 乗して搬送波位相を追跡（**位相は unwrap する**。
`angle()/2` のままだと分岐が跳ぶたびに 1 ビット誤る。最初の実装はこれで 12 dB でも 0.8% の
誤り下限に張り付いた）→半正弦整合フィルタ（2 シンボル幅）で I/Q を判定→差動デコード→
G3RUH デスクランブル→NRZI→HDLC（CRC-16/X.25）。タイミングは 8 位相×2 極性を総当たりし、
**CRC が通ったフレームだけ**を出力する。フレームは CRC を除いたバイト列で返し、AX.25 として
解釈できるか（`decode_ax25`）は既存の処理に任せる。**ARICA-2 のフレームは HDLC/CRC は
AX.25 と同じだが、中身が AX.25 のアドレス部ではない**（先頭 `76 xx`、100 バイト固定）。
Direwolf が「(Not AX.25)」と出すのはこのため。`raw_frame_received` にはそのまま流れる。

**ストリーム処理**: `CoherentMskStream` が I/Q を 2 秒チャンク・1 秒オーバーラップで
処理する（フレームは終了から最大 1 チャンク遅れて出る）。オーバーラップとタイミング位相・
極性の多重デコードによる重複は (内容, 時刻) で除去する（実際に再送された同一フレームは
別物として残る）。信号ゲート（帯域内電力が雑音床の 1.3 倍を超えるチャンクだけ処理）で
無信号時の CPU を節約。速度は実時間の約 15〜20 倍（macOS）。

**エンジン統合**（`AprsEngine`）: 4800/9600 の SDR セッションは、**コヒーレント検波と
Direwolf を並列**に動かし、両方のフレームを `_handle_frame()` で合流・重複除去する
（4 秒窓。コヒーレントが遅れて出すため）。**コヒーレント検波は h=0.5 でないと機能しない**
（合成信号で偏移 ±3 kHz@9600（h=0.62）は 12 dB でも 0/20、±1.8 kHz も 0/20）ので、
従来の弁別器＋Direwolf を捨てられない。Direwolf の実行ファイルが無くてもコヒーレントだけで
動く。`sync_sdr_baud(pipeline, modem, satellite)` は 1200bps のみ `satellite` の違いで
再起動する（4800/9600 では無関係）。

**実測**（`scripts/g3ruh_sensitivity.py`、`--decoders coherent` を追加）:

| 条件 | 従来の最良 | コヒーレント |
|---|---|---|
| 4800bps 合成 50% 点（Eb/N0） | 約 12〜13.5 dB（差動検波・Direwolf・`gmsk_demod`） | **約 8 dB**（10 dB で 28/30、12 dB 以上で 30/30） |
| 9600bps 合成 50% 点（11 kHz 帯 SNR） | 約 12.5 dB（Direwolf）、14 dB（gr-satellites） | **約 8.5 dB**（11 dB 以上で 20/20。60 フラグの前置で 10 dB でも 20/20 vs Direwolf 0/20） |
| 実 ARICA-2（2026-09-24、3 バースト） | Direwolf 最大 4、SatNOGS 1、gr-satellites 4、`gmsk_demod` 4 | **43 フレーム（34 種類）**。バースト 1 の 10 フレームは他局の公開データと一致 |

**注意点・限界**

- 変調指数 h=0.5 専用（上記）。地上の 9600bps など偏移が違う信号は Direwolf 側が拾う。
- 実 9600bps 信号でのコヒーレント検波の検証は**合成信号のみ**（実録音で 9600bps の GMSK が
  デコードできた例がまだない。GRBBeta は信号自体が弱すぎた）。
- 実 4800bps は ARICA-2 の 1 パスのみで調整した。他衛星・他パスでの追加検証が望ましい。
- 1200bps の衛星用前段は OrigamiSat-2 の 1 パスに合わせて調整（クランプ 2.5 kHz など）。
  偏移が大きい衛星（例: ISS の APRS ±5 kHz）ではクランプが効きすぎる恐れがある。
- 4800/9600 の SDR セッションは Direwolf とコヒーレントの 2 つを動かすため CPU 負荷が増える
  （無信号時はゲートで軽い）。
- 「Rig + サウンドカード」経路は従来どおり Direwolf のみ（コヒーレント検波は SDR の生 I/Q が必要）。

**再現手順**: 合成信号 `python scripts/g3ruh_sensitivity.py sweep --snr 12 10 --decoders direwolf coherent`、
実 IQ 録音 `python scripts/g3ruh_sensitivity.py file <x.iq.wav> --decoders coherent`。
テストは `tests/test_coherent_msk.py`（合成 4800/9600 を SNR・搬送波ずれ・ブロックサイズ・
チャンク境界・2.4 Msps で）、`tests/test_afsk_audio_demod.py`（衛星プロファイル）、
`tests/test_aprs_engine.py`（並列セッション・重複除去・再起動条件）。

**教訓**: 複数の独立した復調器（Direwolf / gr-satnogs / GNU Radio / gr-satellites）が同じ
信号で同じ所で頭打ちになるなら、原因は個々の実装ではなく**方式の限界**（ここでは FM 弁別器の
しきい値効果）。「SNR が低い」で結論する前に、公開されている他局の受信データ（フレームの中身）を
正解として、ビット単位の誤り率で方式を比較すること。なお以前の「自前で復調を再実装して失敗した」
教訓（1200bps、2026-09-13）は、検証手段（他局データ・合成信号での感度曲線）が無かった時の話で、
今回は両方を使って原理的な利得（約 4 dB）を確認した上で追加している。

#### SDR経由9600bpsが実機で終日デコードせず — 推測での2回の「根本原因」修正が的外れだった顛末（2026-09-12）

上記のオフライン検証（録音IQ→Direwolf）でリサンプル不一致を修正した後も、ユーザーが
実際にアプリを起動してSDR受信を試すと**9600bpsは依然として一切デコードしなかった**。
「今後は推測で修正せず、必ず診断ログを見てから修正すること・診断ログは動作確認できるまで
削除しないこと」という運用ルールが確立したのはこの調査の最中（ユーザーからの明示的な
指摘2回）であり、以下の(1)(2)は**そのルール確立前に、診断ログの確証なしに「これが原因だろう」
という推測だけで実装し、実際には症状を解消できなかった**修正である。(3)(4)がログの実測値から
特定した真因。

1. **【推測・効果なし】`AudioBridge.run()`のstdout読み取りが受信専用セッションで永久に
   ブロックする問題**（コミット`dfa87b3`）: `self._proc.stdout.read()`のブロッキングループが
   `stop()`後も終了せず、スレッドがゾンビ化し得ることに気づき先に修正した。実在するバグ
   ではあったが、これ単体では9600bpsが復調できない症状は変化しなかった。
2. **【推測・効果なし】`G3ruhSdrDemod(parent=self)`のスレッドまたぎQObject生成問題**
   （`551d5c2`に統合）: コンソールに`QObject: Cannot create children for a parent that is
   in a different thread`という警告が出ていたことから、`AudioBridge.run()`内で
   `G3ruhSdrDemod`に`parent=self`を渡していたのが原因（`AudioBridge`自身の親子関係は
   Qtメインスレッドだが`run()`は`AudioBridge`自身のワーカースレッド上で動く）と判断し
   `parent=self`を削除した。この警告自体は解消したが、これも単体では9600bps復調が
   直らなかった。
3. **【ログの実測値から特定・真因1】`SDRPipeline.unsubscribe()`がバインドメソッドの
   identity比較で一致判定しており、実際には一度もsubscriberを削除できていなかった**
   （`src/sdr/pipeline.py`、コミット`0914f7d`）。Pythonでは`obj.method is obj.method`は
   毎回別オブジェクトが生成されるため`False`になる（`obj.method == obj.method`は`True`）。
   `unsubscribe()`は`c is not callback`でフィルタしていたため、`subscribe()`側の
   `callback not in self._subscribers`（`==`ベース）で追加はできても、`unsubscribe()`側では
   絶対に該当要素を除去できていなかった。この結果、1200↔9600の切り替えや再起動のたびに
   **古い復調セッションが「ゾンビ」として購読リストに残り続け、新しいセッションと同時に
   生のI/Qを受け取り続けていた**（両者が競合し、下流の状態が実質的に壊れていた）。
   Python REPLで`f.bar is f.bar`→`False`・`f.bar == f.bar`→`True`、旧ロジックでは
   `remove`相当が一度も効かないことを直接再現してから修正（推測ではなく実証してから
   のコミット）。`subscribe()`と同じ`==`ベースの比較に統一。
4. **【ログの実測値から特定・真因2】`audio_ready` Signalが`_rx_callback`（プレーンな
   クロージャ、QObjectのメソッドではない）に対して、Qtの自動接続タイプ判定では
   スレッドをまたいだ配送を確実に行えていなかった**（`direwolf.py`、`551d5c2`）。
   診断ログで`G3ruhDiscriminator.process()`は800回以上連続で正常に音声を生成し
   続けている（`empty=0/800`等）のに、`_rx_callback`側の呼び出しカウンタは**最初の
   1回で止まったまま増えない**ことを確認し、初めて「復調自体は成功しているのに
   Direwolfへ音声が届いていない」という真因を特定した。
   `sdr_demod.audio_ready.connect(_rx_callback, Qt.ConnectionType.DirectConnection)`
   と明示することで解消。

**この4件すべてを適用した状態で、ユーザーが実機で9600bpsのデコード成功を確認済み**
（2026-09-12）。(1)(2)は単体では無効だったが、(3)(4)と合わせて動作する状態になった
ため撤回はしていない（実在するバグの修正ではあった）。診断ログ（`push_samples()`・
`run()`の受信/処理カウンタ、`_rx_callback`呼び出しカウンタ等）はユーザー確認後も
現時点では削除していない。
...
`ARATE`の値そのものが正しくても、実際に供給する音声の実レートが一致していなければ
同じ症状になる——2026-09-12に見つかった`G3ruhDiscriminator`のリサンプル不一致
（上記「SDR単体経由（9600bps）実信号での検証完了」参照）がまさにこのパターン。

#### 1200bps SDR単体経路（`AfskDemodulator`）の実装不備の発見・修正（2026-09-12）

ユーザーがRTL-SDR経由でAPRS受信を試したところ、無線機+サウンドカード経由（Direwolf）は
正常デコードするのに、SDR単体経由（`AfskDemodulator`、`afsk_demod.py`）は**一度も
デコードしない**という報告があった。周波数・PPM（1ppm、問題なし）を確認した上で
コードを精査したところ、独立した3つのバグが見つかった。いずれも「クロック同期の
追加」という当初の依頼を実装・検証する過程で発覚したもので、3つとも修正しないと
デコードは一切成立しない:

1. **シンボルクロック同期（PLL）が存在しなかった**: 元の実装は固定のブロック内グリッド
   （`i*sym_samples+sym_samples//2`）でトーンをサンプリングしており、送信側（衛星や
   デジピーター）の独立したクロックに追従する仕組みが皆無だった。トーン遷移
   （＝必ずシンボル境界で発生）を検出するたびにサンプリング位相を補正する
   デジタルPLLを追加。
2. **`_HdlcState.push_bit()`のビットスタッフィング処理バグ**: 閉じフラグ（0x7E）
   自身が持つ6連続の1ビットが、5連続超過のビットスタッフィング違反として誤って
   アボート扱いされ、シフトレジスタがフラグを認識する1ビット前に必ずフレームを
   破棄していた。**信号品質に関係なく、正常な閉じフラグ経由では原理的に一度も
   フレームを返せない**という、実装当初から存在していた致命的バグ。アボート閾値を
   6連続（フラグ由来）と7連続以上（真の違反）を区別するよう修正。
3. **ボックスフィルタのブロック境界破損**: 1シンボル幅の平滑化に
   `np.convolve(mode="same")`を`push_samples()`呼び出しごとに単独計算していたため、
   SDRPipelineの1ブロック（16384サンプル、約15〜65ms相当）境界ごとに暗黙のゼロ
   パディングで数サンプル分が破損していた。実際のAPRSパケット長（数百ms）はブロック
   境界を複数回またぐため、ほぼ必ずどこかで壊れていた。履歴を`_box_state`として
   次回呼び出しに引き継ぐストリーミング方式に変更。

さらに、これら3件を修正した後も実受信でデコードしないとの再報告を受けて調査したところ、
RTL-SDRのLO自己混信によるDCスパイクが同調中心周波数（＝復調に使う搬送波の位置）に
直接乗ることが判明。SDR ControlタブのNFM音声復調パス（`sdr/demodulator.py`の
`Demodulator._remove_dc()`）は既にこれを除去していたが、`AfskDemodulator`は生I/Qを
1段階手前（`pipeline.subscribe()`）でタップしているためこの除去処理を受けておらず、
「音は綺麗に聞こえるのにビット単位のデコードだけ失敗する」症状と一致した。同じ
30Hzハイパスフィルタ設計を追加。

合成信号（送信機クロック誤差±200ppm・DCオフセット注入・実際のSDRPipelineと同じ
16384サンプル単位のチャンク処理）で検証済み。ただし2026-09-12時点で**1200bpsの
実信号での受信確認はまだ完了していない**（後述「地上系でのBaud「自動」の制約」参照
——テスト時に9600bps信号を誤って1200bpsとして処理しようとしていた可能性があり、
1200bpsパケットの実受信自体の待機・再確認が必要）。

コミット: `3524645`（PLL・HDLC・ボックスフィルタ）、`20b59cf`（DC除去）。

#### 瞬時周波数への固定しきい値比較が原理的に誤りと判明・mark/space電力比較へ置き換え（2026-09-12、未検証）

上記3件＋DC除去を修正した後も、ユーザーは実際に144.66MHzで1200bps信号を受信し続けたが
依然として一度もデコードしなかった。`_HdlcState.push_bit()`の診断ログ（フラグ検出の
たびに`was_in_frame`・直前フレーム長・16進プレビューを毎回記録するもの）を見ると、
**ほぼ毎回のフラグ検出直後で`was_in_frame=False`**（＝フラグとフラグの間にまともな
ペイロードが全く蓄積されていない）というパターンが継続していた。この時点で一度
「そもそも受信ウィンドウに本物のパケットが無い（ノイズ）のでは」という仮説を出したが、
**ユーザーから「1200の信号の山を波形上で何度も目視確認済みであり、ノイズではなく
コードのバグである」と明確に否定された**。以後は推測を避け、診断ログの実測値のみに
基づいて調査する方針に切り替えた。

`_process()`に、1シンボル分平滑化した瞬時周波数（`smoothed`、当時のトーン判定に
使っていた値そのもの）の分布統計（min/mean/max/std）と、しきい値1700Hzを超える
サンプル割合（`space_frac`）を記録する診断ログを追加したところ、以下が繰り返し
観測された:

- `smoothed`は**0Hz付近を中心に概ね±3600Hz程度で左右対称**に分布（meanがほぼ0）
- `space_frac`（1700Hz超の割合）は**2〜9%程度**にとどまり続けた

AX.25のNRZI＋ビットスタッフィング済みビット列であれば、mark/space両トーンの
出現時間は概ね拮抗するはずであり、0Hz中心の左右対称分布・space側2〜9%という値は
「本物の1200/2200Hzの2値信号」というモデルと矛盾する。これは、無線機のFM変調を
経由したBell 202信号をSDR側で位相差検波すると、**その出力（瞬時周波数）は
"1200"や"2200"という値そのものにはならず、無線機自身のFM偏移量（数kHzオーダー、
未知・機種依存）を振幅として、トーン自身の周波数（1200Hzまたは2200Hz）で振動する
交流波形になる**ためと判断した。従来の実装（`smoothed >= 1700Hz`という固定レベル
しきい値判定）は、この交流波形ではなく「トーンごとにinst_freqがほぼ一定値に張り付く
直接FSKベースバンド信号」を暗黙に前提としており、**Bell 202-over-FM信号に対しては
原理的に成立しない**アルゴリズムだった（無線機+サウンドカード経由の1200bpsが
既に成功している事実と矛盾しない — Direwolf自身のAFSKデコーダーは元々レベル
しきい値ではなく、まさにこの交流波形に対する1200Hz/2200Hzの帯域通過フィルタ＋
電力比較〈Goertzel方式〉を使っており、今回のSDR側の修正はDirewolfと同じ検出原理に
揃えるものである）。

**修正**（コミット`34586cc`）: `_process()`のトーン判定を、瞬時周波数への固定
しきい値比較から、1200Hz・2200Hzそれぞれへの直交ミキシング（`inst_freq * exp(-j2πf_tone·n/Fs)`、
ローカル発振器の位相はブロック境界をまたいで連続保持）＋1シンボル幅のストリーミング
ローパス（既存のボックスフィルタ状態パターンをcomplex型に拡張）による`mark_power`/
`space_power`比較（`space_power > mark_power`）に置き換えた。PLL・HDLC・残差処理の
インターフェース（`tones`をサンプル単位のbool配列として渡す契約）は変更していない。
診断ログは削除せず、`smoothed`の分布統計の代わりに`mark_power`/`space_power`それぞれの
min/mean/max・`space_frac`・トーン遷移回数を記録する内容に更新した。

**2026-09-12時点のステータス: 未検証。** ユーザーの実受信試験は翌日（2026-09-13）
実施予定であり、本修正が実際に1200bpsのデコードを成立させるかどうかはまだ
確認できていない。診断ログは実受信での動作確認が取れるまで削除しないこと
（`feedback_keep_diag_logs_until_fixed`メモリ参照）。

#### 地上系（衛星未選択）でのBaud「自動」の制約（2026-09-12）

`resolve_ax25_modem()`（`engine.py`）の「自動」判定は、**受信信号を解析して1200/9600を
判別する仕組みではない**。Radio Controlタブで現在選択中の衛星トランスポンダーの
SATNOGS DB上の`baud`列を読むだけであり、地上系受信（衛星・トランスポンダー未選択、
`current_transmitter()`が`None`）の場合は`xpdr is None`となり**無条件に1200bpsへ
フォールバックする**。

このため、地上系で9600bps（例: JAPRSデジピーター網の144.64MHz）を受信する場合、
Baudを「自動」のままにしておくと常に1200bps（`start_sdr_direwolf(modem="1200")`）が
起動してしまい、9600bpsのG3RUH信号は原理的にデコードできない。**Baudを手動で「9600」
（または該当するレート）に選択する必要がある。**

信号ベースの自動判定（受信IQから実際に1200/9600を判別するロジック）は未実装。
今回はドキュメント化のみとし、実装は見送った。将来対応する場合は
`aprs_tab.py`/`telemetry_tab.py`のBaudコンボの「自動」選択時の挙動として、
1200/9600それぞれのDirewolfセッションを短時間並行起動して信号の有無で判定する、
等の設計が考えられる。

#### 1200bpsもDirewolf経由方式へ全面移行（2026-09-13、実受信で解決確認済み）

上記の`AfskDemodulator`（mark/space電力比較方式）修正後も、ユーザーが実受信で試した
ところ依然デコードしなかった。録音したIQのオフライン解析で、診断ログに新設した
「間引き前の生の瞬時周波数」統計から、実信号に**約-20,200Hzの一定したオフセット**が
繰り返し現れることを発見し、一度は「144.64MHzの9600bps信号（20kHz離れているだけで
250kHz受信帯域内に同時に入る）が漏れ込んでいるのでは」という仮説を立てて帯域通過
フィルタ追加を提案した。しかし**ユーザーがスペクトラムを直接確認した結果、
9600bpsと1200bpsが同時に発射されている場面はほとんどなく、1200bpsだけを受信して
いることが明確な状況でもデコードできていない、との指摘があり、この仮説は撤回した**
（実際のオフセットの正体は追跡していない）。

ここでユーザーから決定的な方向転換の指示があった: **「そもそも、direwolfに正しい
信号が届いていない、というのが最も可能性が高いはずです。direwolfにまかせておけば、
普通にデコードできるはずなのに、なぜ、余計なことをいろいろしようとするのか。
余計なことをせず、direwolfに正しく入力することを考えろ」**。無線機＋サウンドカード
経由の1200bpsは終始問題なくDirewolf本家でデコードできていたという事実（このセッション
全体を通じて一度も揺らいでいない）を踏まえると、**自前でBell202のトーン検出・PLL・
HDLCを再実装し続けること自体が誤ったアプローチだった**、という指摘だった。

**最終的な設計変更**: 9600bps G3RUH（`g3ruh_demod.py`）が既に採用している「SDRの生I/Q
から適切な音声を復調し、Direwolf本家のデコーダーに渡す」という方式に、1200bpsも統一した。

- 新規`src/comms/aprs/afsk_audio_demod.py`: `G3ruhDiscriminator`と全く同じ構造
  （DC除去→中間レート200kHzで位相差検波→正確な48kHzへの有理数リサンプル）に、
  **de-emphasis段を追加**した`AfskAudioDiscriminator`/`AfskAudioSdrDemod`。G3RUHは
  de-emphasisなしの生判別器出力が必要なのに対し、Bell202のトーンは無線機の通常の
  音声経路（de-emphasis込み）を通るため、ここが唯一の設計上の違い
- `direwolf.py`の`AudioBridge`がmodemに応じて`AfskAudioSdrDemod`（1200）／
  `G3ruhSdrDemod`（4800・9600）を選択するよう変更
- `engine.py`の`start_sdr()`（自前Python実装の呼び出し口）を削除し、1200bpsも
  `start_sdr_direwolf(modem="1200")`に統一。`sync_sdr_baud()`も「2つの復調
  メカニズムを切り替える」ロジックから「同一メカニズム内でmodemを変更する」だけの
  単純な形に整理
- 自前実装だった`src/comms/aprs/afsk_demod.py`（HDLC・PLL・トーン検出・診断ログ一式）は、
  アプリのどこからも呼ばれなくなったため削除
- `tests/test_aprs_engine.py`をDirewolf経由方式のみを前提としたテストに更新

**結果**: 2026-09-13、ユーザーの実受信試験で1200bps・9600bpsとも問題なくデコード
できることを確認。この時点でこのセッション全体を通じて追加していた一時的診断ログ
（`afsk_audio_demod.py`・`direwolf.py`・`engine.py`・`g3ruh_demod.py`の`TEMPORARY
diagnostic`マーク付きログ）はすべて削除した（恒常的な監視用ログ——キュー溢れ検出・
stdin書き込み失敗・KISS接続断など——は残している）。

**教訓**: 実際の無線機＋サウンドカード経由で確実に動作している既存の実装（この場合
Direwolf本家のAFSKデコーダー）がある場合、SDR側で同じプロトコルを自前のPythonで
再実装しようとする前に、まず「無線機側で動いているものと同じソフトウェアに、
正しい入力を渡す」設計を検討すべきだった。今回は約1日半にわたる自前DSP実装の
デバッグ（PLL・HDLC・DC除去・間引き順序・帯域通過フィルタ仮説など多数の修正）の
末にこの結論に至ったが、後から振り返ると9600bps G3RUHの実装（同じ「Direwolfに
正しい音声を渡す」方式）が最初から存在し、かつ実績もあったため、より早い段階で
同じ方式への統一を検討する余地があった。

**未解決の課題（オフライン検証の限界）**: この修正をユーザーの実受信より前に自力で
検証しようと、合成音声を実際のDirewolfサブプロセスへ`ADEVICE stdin stdout`経由で
直接流し込むテストスクリプトを試みたが、ゼロ交差法で正しいと確認済みの理想的な
Bell202音声ですら一度もデコードできず、原因を特定できないまま終わった（8回繰り返し
配信・実時間ペースでの分割書き込みなど複数のバリエーションを試したが全滅）。
一方、9600bps G3RUHは同じ`ADEVICE stdin stdout`方式で実際にライブ動作が確認済みで
あるため、これは仕組み自体の欠陥ではなく即席スクリプト側の未発見の設定ミスである
可能性が高いと判断し、それ以上の追求を保留した。同種のオフライン検証を今後試みる際は
この制約を踏まえること。
...
**重要**: FT4 の**送信（QSO）にはリグ（トランシーバー）が必須**。**受信のみなら SDR 単体で可能**（2026-09-19 以降。下記「SDR 経由の FT4 受信」参照。それ以前は SDR 入力の選択肢があっても実際にはデコードできなかった）。

#### SDR 経由の FT4 受信（2026-09-19 修正）

**症状/原因**: FT4 タブに SDR 入力（`rx_source == "sdr"`）の選択肢があったが、SDR では
**一度もデコードできなかった**。`_on_sdr_audio_chunk()` が SDR パイプラインの
`audio_ready`（公称48 kHz）を**そのまま**受信バッファへ入れており、(1) FT4 デコーダー
（libft4wsjt）は12 kHzを要求し「リサンプルは呼び出し側の責任」（`wsjt_decoder.decode_audio`
は12 kHz以外だと即 `[]`）なのに48→12 kHz変換が無く（サウンドカード経路は共有入力管理が
変換していた）、音は4倍に間延びして1周期の先頭1.5秒分しか入らなかった、(2) さらに
`Demodulator` の音声は実際のレートが48 kHzではなく（250 kS/sのUSBで62.5 kHz。同日別途
修正）、(3) SDR ControlのUSB復調は音を周波数のまま出さず**SSB帯域幅の半分（1350 Hz）だけ
下へずらす**（2000 Hzの信号が650 Hz。実数部を取るため下半分は鏡像にもなる。**この復調自体も同日中に修正済み**で、現在はUSB/LSBとも本物のSSBとして音程が保たれる。[sdr.md](sdr.md)「SSB(USB/LSB)復調が本物の SSB になっていなかった」参照。FT4は12 kHz要求などがあるため専用経路のまま）。
FT4 は音声トーンが「ダイヤル周波数からの真のオーディオ周波数」にあることが前提なので、
たとえ(1)(2)を直してもこの復調は使えない。

**修正**: FT4 タブは復調済み音声ではなく**SDRのI/Q**（`SDRPipeline.subscribe()`、Doppler
補正後なので0 Hz＝同調したダイヤル周波数）を直接受け取り、専用に12 kHzのUSB音声へ変換する
（`src/sdr/usb_audio.py` の `UsbAudio12k` / `SdrUsbAudioTap`）:
I/Q →（多段のアンチエイリアス＋間引きで約50 kS/s）→ −1750 Hzミックス＋複素LPF（±1.9 kHz）
で**上側波帯だけ**（0〜約3.5 kHz。反対側波帯は除去）→ 12 kHzへ厳密リサンプル →
+1750 Hzミックスして実数部を取る。トーンは真の周波数（1000 Hz→1000 Hz）で出る。全フィルタ・
オシレータ・リサンプラは状態をブロック間で引き継ぐ。SDRの利得に依らないよう、平均電力を
一定にする遅いAGC（時定数4秒）を後段に入れている。処理は専用スレッドで、パイプラインの
読み出しループは止めない（追いつかなければブロックを捨てる）。

**検証**（合成した実際のFT4送信をSDRのI/Q〔解析信号を250 kS/sへ〕にして通す）:
- 修正前の経路（`Demodulator` USB → そのまま）: 雑音無しでも**デコード0**
- 新経路: 400 / 1500 / 2800 Hz のいずれもデコード成功。SNR（2500 Hz換算）0〜−14 dBで4メッセージ
  ×3雑音の12通りを試し、**12/12（−14 dBでも。直接12 kHz音声の基準は10/12）**。SDRの
  サンプルレート 250 k / 960 k / 1.024 M / 2.4 MS/sのいずれも成功。つまりサウンドカード
  経由と同等の感度
- 実機（実SDR＋実際のFT4衛星信号）での確認は**未実施**

副産物: SDRパイプラインを再生用の疑似デバイス（IQ録音の再生、`docs/sdr.md`）に差し替えても
同じ経路が働くので、録音したIQからFT4をデコードすることもできる。Q65タブも同じ構造
（`SAMPLE_RATE=12000`・SDR音声を変換せず投入）で同じ欠陥を持つが、**今回は未対応**。
テスト: `tests/test_usb_audio.py`（レート・周波数の真値・反対側波帯除去・ブロック非依存・
レベル正規化・タップのスレッド動作・FT4のI/Qエンドツーエンド〔ライブラリ無しならskip〕）、
`tests/test_ft4_sdr_source.py`（FT4タブの接続/切断と12 kHzでの受信バッファへの到達）。
