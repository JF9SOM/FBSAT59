...
#### デコードに必要な最低SNRの目安（2026-09-19 整理。Telemetryタブの ⓘ ボタンの元データ）

Telemetryタブの入力ソース行の右端に **ⓘ ボタン**を置き、クリックでこの表を要約した
ウィンドウ（`_SnrGuideDialog`、`ui/telemetry_tab.py` の `_SNR_GUIDE_ROWS` / `_snr_guide_html()`）
を開く。数値は下記の**合成信号での測定**で、実衛星では変調・偏移・フレーム長が異なる目安。
