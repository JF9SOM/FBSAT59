...
2. **IF が広すぎた（±13 kHz）**。9600bpsの信号は約±6 kHzに収まるが、FM弁別器の出力雑音は
   ...
   **IF半帯域 7.5 kHz + 弁別後LPF 6.5 kHz（101タップ）**を採用（±2 kHzずれでもSNR
   ...
   9600bpsのみ変更（`_Profile`テーブル、`G3ruhDiscriminator(baud=...)`）。**4800bpsは
   従来のIF（±13 kHz）のまま**にした。合成では4800も±4 kHz程度に狭めて約5 dB改善するが、
   周波数誤差許容度が±1〜1.5 kHz程度に下がること、および実信号での確認手段が無いことから
   保留した（なお、この時点で4800bpsが実信号で復号に成功した記録は無い。ARICA-2の検証は
   `ARATE`必須の発見までで、フレーム復号成功の記録ではない）。ただし状態引き継ぎは4800にも
   効く（合成4800、SNR 24 dBで18〜27/40→40/40、SNR 16 dBで1〜12/40→18〜38/40）。
...
ブロックサイズ非依存、狭IFが帯域外の強い干渉波を除去する（4800のレガシーIFは通す）こと、
信号帯域（±4 kHz）を正しく通すこと。ブロック単位のリサンプル（`resample_poly`）は
