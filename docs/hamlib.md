...
### catch-up中の位置確認ポーリングを先読み時間経過まで抑制（2026-09-16、同日追加）

**きっかけ**: 最大仰角70度のN→S・N跨ぎパスを実機テストした際、ユーザーから
「初回jumpと同様に、北跨ぎのcatch-upも位置更新・位置確認をCycle周期（実測2秒）ごとに
行うのをやめ、目標地点まで一気に回らせられないか」との指摘。実際には初回jump・0度またぎ
再突入とも**同じ状態機械**（`_catching_up`フラグ）を共有しており、catch-up中は`_send_p()`
（Pコマンド送信）こそ1回きりだが、**目標に届いたかを確認する`get_position()`ポーリングは
Cycle周期のまま毎サイクル行われていた**（Pコマンドは出ていないので実害は未確認だったが、
「読み取りだけだから無害」という前提自体、この機種のシリアルプロトコルの実際の挙動では
確認できていない）。

**修正**: `_lead_target()`が既に計算している先読み時間（`lead_s`）を`self._catch_up_lead_s`
として保持するようにし、catch-up開始（`_catch_up_start_time`）からこの時間が経過するまでは
`set_position()`の到達確認自体（`get_position()`呼び出し含む）を丸ごとスキップするよう変更。
初回jump・0度またぎ再突入のどちらも同じ`_lead_target()`経由でこの値がセットされるため、
自動的に両方に適用される。先読みなし（predictor未設定、または予測失敗）の場合は
`_catch_up_lead_s=0.0`となり、従来通り毎サイクル確認する（回帰なし）。

**60秒タイムアウトとの整合性**: `_CATCH_UP_LEAD_MAX_S`は120秒だが`_CATCH_UP_TIMEOUT`は
60秒のため、先読み時間が60秒を超えるケースでは、従来のロジックのままだと**まだ正当に
移動中のはずの先読み目標に対して、位置確認すら行わないうちにタイムアウト誤発動**しうる
矛盾があった（今回の変更以前から潜在していた不整合）。タイムアウト判定を
`elapsed > max(_CATCH_UP_TIMEOUT, self._catch_up_lead_s)`に変更し、自分で見積もった
先読み時間より早く「タイムアウト」とは判定しないよう合わせて修正した。
