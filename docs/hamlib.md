...
残す必要性自体がなくなった）。**2026-09-18に撤回**: 後述「crossed_zero経路での北跨ぎ
catch-upが実機で無反応になる不具合の調査」で、開始・終了の2点だけでは実機が本当に
動いていたかを事後判定できないことが判明したため、catch-up中は毎サイクル`rot_az`/`rot_el`/
目標AZ/wrap起点AZ/経過時間を`rot-record.log`へ記録するようにした（診断目的の恒久ログ、
動作変更なし）。
...
### crossed_zero経路での北跨ぎcatch-upが実機で無反応になる不具合の調査（2026-09-18 追加）

**症状**: South Init OFF のまま、AZが真北（0/360）をまたぐパス（AOS AZ=8°→LOS AZ=204°、
最大仰角61.1°）を追尾中、通常追尾から`crossed_zero`経由でcatch-upに再突入した直後、
ローテーターが完全に無反応になった（ユーザー実機確認: モーター音・アンテナの向きとも
一切変化なし）。`fbsat59.log`のタイムライン:

```
07:50:22  initial jump from origin=0.0 to az=0.6 el=21.0
07:50:23  caught up at rot=0.4 target=0.6 sat=0.6
07:50:31  0-degree wrap 0.1->360.0, rotor_origin=0.1, re-entering catch-up toward az=343.3 el=43.5
（以降、アプリ終了(07:51:16)まで45秒間、catch-up成立ログもエラーログも一切出ない）
```

**わかったこと**: `_lead_target()`は起点(0.1)と目標(343.3)が0/360の縫い目をまたぐと判定し
（`boundary_forced=True`）、最短経路ではなく長い方の経路（約359.5度、想定スルー速度
3.29度/秒で約110秒）を先読み距離として採用、`_send_p(343.3, 43.5)`を送信していた
——ここまではこれまでの修正群（本ファイル前節群）の設計どおり。一方、これまで
`rot-record.log`に残っている「長距離の強制迂回が実機で成功した」記録
（distance=303.7°・169.7°・75.9°、いずれも2026-09-17）は**すべて接続直後の初回jump
経路**のものであり、**通常追尾中に`crossed_zero`を経由して再突入した経路からの
長距離迂回が実機で成功した記録は存在しなかった**（先読みを`crossed_zero`経路にも
適用する変更自体は本ファイル前節の通り2026-09-17に追加済みだが、この組み合わせ・
distanceでの実機確認は今回が初めてだった）。

**確定できなかったこと**: `_send_p()`は`self._rot.set_position(az, el)`（python-hamlib
バインディング）の戻り値・エラー状態を一切チェックしておらず、例外も出ていない
（＝アプリ側は「送信成功」と認識したまま）。しかし`rot-record.log`には**catch-up中の
ローテーター実位置がこれまで記録されていなかった**ため、今回の45秒間に実機側で
本当に回転が始まっていたのか、Pコマンド自体が実機に届いていなかったのかを、
ログから事後に判定する手段がなかった。

**対応（ユーザー判断、2026-09-18）**: 動作は変更せず、`set_position()`の
catch-up継続チェック（`if self._catching_up:`、`crossed_zero`再突入後の待機も含む）
内で、`get_position()`の実読み取り値（`rot_az`/`rot_el`）・目標AZ（`self._last_az`）・
0度またぎ起点AZ（`_catch_up_wrap_origin_az`、ゲート中のみ）を、**毎サイクル**
`rot-record.log`へ記録するようにした（前節「診断ログの分離」の「毎サイクルの位置
ポーリングは残さない」方針を撤回）。次に同じ症状が再現した際、`rot_az`が時間経過
とともに実際に動いているかどうかで、「Pコマンドが実機に効いていない」のか「単に
想定より遅い／別の理由で足止めされている」のかを切り分けられる（経過時間は
`rot-record.log`のタイムスタンプ間隔から算出できるため、ログ項目自体には含めていない
——`time.monotonic()`の追加呼び出しは既存テストのモック呼び出し順序を壊すため見送った）。

South Init を ON にすればこのパス自体は北をまたがなくなり本事象を回避できるが、
それとは別に「South Init OFF でも北跨ぎを正しく検出し高速で反対回りに追いつく」という
...
- **途中切断**: `set_position`（`_send_p`）と `get_position` の結果を `_record_io_result()` で
  監視し、**失敗が成功を挟まず 30 秒間続いたときだけ**（`_UNREACHABLE_AFTER_S`）接続を閉じて
  `UNREACHABLE` にする。それより短い失敗（一時的なシリアルの不調・ローテーターのビジー等）は
  無視して状態を変えない。成功が 1 回でもあれば失敗の連続はリセットされ、失敗どうしの間が
  30 秒を超えた場合も新しい連続として数え直す。判定材料は Direct=`error_status`、
  NET=空応答/`RPRT <負>`/送信時の `OSError`。失敗した読み取りのゴミ値は `_rotor_state` に
  反映しない。失敗が始まった時点で警告ログ（Hamlib のエラーコード付き）を 1 回出す。
  （当初は「連続 3 回失敗」だったが、正常な機器でも一時的に非 0 が返る場合に追尾が止まる
  リスクがあるため、2026-09-19 に時間ベースへ変更した。）
...
- **手動 Connect での「未接続」表示は実機確認済み**（2026-09-19、ローテーター未接続・
  ...
  追尾中に途中切断の判定が誤って働かないことは未検証。** 30 秒続かない限り発動しないが、
  誤って「未接続」になった場合は `fbsat59.log` の `Rotator: ... failed — will drop the
  connection ...`（Hamlib エラーコード付き）を確認すること。
...
### 高仰角（天頂通過）パスの「迎撃」先読み（2026-09-25 追加・実機未検証）

**症状（2026-09-25 15:39 KOSEN-2R、最大仰角約87度）**: TCA 前後で指令 AZ が 4 秒で最大 54 度（約 13 度/秒）動いた
（15:39:40 58° → 44 秒 112° → 48 秒 155°）が、ローテーターの実スルー速度は 2.3〜4.1 度/秒（`rot-record.log`）。
毎サイクル現在位置を再送する方式では約 1 分遅れる。さらに毎サイクルの P 再送は SkyWatcher のモーター加速を
その都度やり直させる可能性があり、実効速度も落ちる（15:39:44〜15:40:48 に P コマンドが 1 回おきに失敗）。
この 176° の変化は 0 度またぎではないので、既存の wrap/先読みの対象外だった。

**実装**: `HamlibRotatorController._try_fast_pass_intercept()`（通常追尾の直前に呼ぶ）。
1. 5 秒先の衛星 AZ が `想定スルー速度 × 5` 度より離れる場合だけ発動（普通のパスは一切影響なし）。
2. 2 秒刻みで最大 180 秒先まで走査し、「ローテーターが t 秒で届く（距離 ÷ 想定速度 ≤ t）」最初の t の衛星位置を求める。
3. その 1 点へ P を 1 回だけ送り、既存の catch-up 状態機械に入る（到達まで再送しない。到達したら通常追尾に戻る）。
- 発動しない条件: 予測器なし／迎撃点が水平線下／走査が 0/360 の継ぎ目に絡む（既存の wrap 処理が担当）／
  必要な先読みが 6 秒未満または回転量が 15 度未満（ループ防止）。
- catch-up 経路を使うため `rot-record.log` に到達時の実速度も記録され、想定スルー速度の自動較正にも使われる。
- あわせて追尾中のログ `Rotator: set position ... (actual az=... el=...)` と、拒否された P の返答
  （`P az el rejected, reply=...`）を追加した。
