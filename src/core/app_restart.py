"""Restart the running FBSAT59 process (used by the language switch).
...
callers such as ui/main_window.py must be able to trigger a restart
without re-running any of that.
"""
...
def restart_application() -> None:
    ...
    Used by the "restart to apply the language change" prompt.
    ...
    any Communications tab subprocesses, etc.) -- that's why main.py's
    _acquire_single_instance_lock() retries for a few seconds when
    RESTART_ENV_VAR=1 is set, rather than this function waiting on the old
    process's PID itself.
    ...
    script / terminal session tears down, the child gets SIGHUP'd along
    with it -- the app was observed to just quit instead of relaunching.
    Detaching (start_new_session on POSIX, a new process group on Windows)
    and redirecting stdio away from the parent's (about-to-vanish) handles
    avoids that.
    """
